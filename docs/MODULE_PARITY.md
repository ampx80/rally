...
|---|---|---|---|---|---|---|---|
| **Email** | 2 (real Resend send in `api/broadcast.js` + `_lib-email.js`; plain-text only; open/click simulated; browser-bound sequence runner) | 3 | 3 | 2 | 2 | Visual builder, real segments, scheduling on server cron, deliverability (SPF/DKIM/DMARC guidance + suppression + one-click unsub), real open/click via Resend webhooks, per-recipient timeline | **1** |
