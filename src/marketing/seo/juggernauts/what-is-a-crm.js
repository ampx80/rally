...
const entry = {
  ...
  metaTitle: 'What Is a CRM? The Complete 2026 Guide (with Diagrams and a Calculator) | Ardovo',
  ...
  blocks: [
    ...
    {
      type: 'richText',
      title: 'Where CRM is heading, and where Ardovo fits',
      body: [
        ...
        'This is the thesis behind Ardovo. It is an AI-native CRM and revenue platform that is alive on first load, so you see a working pipeline instead of a blank setup wizard. It runs on one flat price across every module rather than a seat-plus-add-on maze, and it ships with an AI operator, Rook, that handles the follow-up and forecasting busywork teams otherwise skip. You do not need Ardovo to benefit from this guide, but if the pattern above is what you want, it is what Ardovo was built to be.',
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'How much does a CRM cost?', a: 'It ranges widely, from free entry tiers to enterprise contracts that run into six figures. The trap to avoid is per-seat pricing stacked with paid add-ons for the features you actually need, because that model gets more expensive exactly as you grow. A single flat price across all modules, like Ardovo offers, is far easier to budget from five users to fifty.' },
        { q: 'What is an AI CRM, and is it different?', a: 'An AI-native CRM does the work rather than just storing it. Instead of waiting for a human to log activity and remember to follow up, it captures automatically, enriches records, flags deals going cold, drafts outreach, and rolls up forecasts on its own. Ardovo is built around this idea, with an AI operator called Rook that handles the busywork a traditional CRM leaves to people.' },
      ],
    },
  ],
