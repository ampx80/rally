// migrations dataset - one "migrate from X to Rally" playbook per tool. Real,
...
const TRANSFERS = {
  crm: [
    ...
    'Custom fields mapped to matching Rally custom fields',
  ],
  sheet: [
    'Every contact and company row becomes a Rally record',
    'Deal rows with stage, value, and close date if your sheet tracks them',
    'Any custom columns mapped to Rally custom fields',
  ...
  marketing: [
    ...
    'Tags and list membership mapped to Rally fields or segments',
    'Custom fields mapped to matching Rally custom fields',
  ],
  engagement: [
...
const MIG = {
  salesforce: {
    ...
    gotchas: [
      ...
      'Apex triggers, flows, and validation rules do not transfer. List your active automations first, then rebuild the ones you still need as Rally workflows.',
  ...
  hubspot: {
    ...
    gotchas: [
      'HubSpot workflows and sequences do not export as files. Screenshot or list your active ones, then rebuild them in Rally.',
      'Deal stages are pipeline-specific in HubSpot, so map each pipeline to a Rally pipeline during import.',
  ...
  zoho: {
    ...
    gotchas: [
      ...
      'Zoho blueprints and workflow rules do not transfer; rebuild the active ones as Rally workflows.',
      'Layout-specific and multi-module fields need mapping care so values land on the right Rally object.',
    ],
  ...
  pipedrive: {
    ...
    gotchas: [
      'Pipedrive custom fields are per-object; confirm each maps to the right Rally object (company vs contact vs deal).',
      'Pipedrive automations and Smart Docs do not transfer; rebuild active automations in Rally.',
  ...
  netsuite: {
    ...
    gotchas: [
      ...
      'SuiteFlow automations and scripts do not transfer; rebuild the sales-side ones in Rally.',
      'Permissions and role complexity in NetSuite do not map one to one, so plan Rally RBAC fresh.',
    ],
    timelineExtra: 'Front-office data moves in about a week; keep NetSuite for finance and let Rally own the revenue workflow.',
  },
  monday: {
    name: 'monday CRM',
    kind: 'crm',
    ...
    gotchas: [
      ...
      'monday automations (recipes) do not transfer; rebuild the sales ones as Rally workflows.',
  ...
  'microsoft-dynamics': {
    ...
    gotchas: [
      'Dynamics option-set (choice) fields export as labels or values; standardize them before mapping to Rally.',
      'Power Automate flows and business rules do not transfer; rebuild the active ones in Rally.',
      'Multi-entity relationships and custom entities need a clear mapping to Rally objects up front.',
    ],
  ...
  close: {
    ...
    gotchas: [
      'Close nests contacts under leads; flatten that into companies and contacts for Rally on the way in.',
      'Call recordings and SMS threads are Close-specific; export what you must keep before cutover.',
      'Close sequences and Workflows do not transfer; rebuild them as Rally sequences.',
    ],
  ...
  copper: {
    ...
    gotchas: [
      ...
      'Copper workflow automations do not transfer; rebuild active ones in Rally.',
      'Custom fields are per-object; confirm mapping to the right Rally object.',
    ],
  ...
  insightly: {
    ...
    gotchas: [
      'Insightly links projects to opportunities; decide whether project delivery moves into Rally projects or stays separate.',
      'Insightly workflow automations do not transfer; rebuild the active ones in Rally.',
  ...
  keap: {
    ...
    gotchas: [
      'Keap campaign builder automations are complex and do not transfer; document the active ones, then rebuild in Rally.',
      'Keap tags carry a lot of meaning; map the important ones to Rally fields or segments.',
  ...
  freshsales: {
    ...
    gotchas: [
      'Freshsales workflows and sales sequences do not transfer; rebuild the active ones in Rally.',
      'If you also use Freshdesk, decide which system owns which contact fields before export.',
      'Deal stages are pipeline-specific; map each pipeline to a Rally pipeline.',
    ],
  ...
  sugarcrm: {
    ...
    gotchas: [
      'Sugar customizations (Studio fields, logic hooks) do not transfer; rebuild the active logic as Rally workflows.',
      'On-prem Sugar may hold years of history; decide how far back you need to bring records.',
      'Module relationships need clear mapping to Rally objects.',
    ],
  ...
  'zendesk-sell': {
    ...
    gotchas: [
      ...
      'Smart Lists and automations do not transfer; rebuild the active ones in Rally.',
  ...
  excel: {
    ...
    gotchas: [
      ...
      'Duplicate rows are common in spreadsheets; Rally dedupes on email and domain, but tidy obvious duplicates first.',
    ],
    timelineExtra: 'A clean spreadsheet can be live in Rally the same day; the time is in tidying the data, not the import.',
  },
  'google-sheets': {
    name: 'Google Sheets',
    kind: 'sheet',
    ...
    gotchas: [
      ...
      'Deduplicate obvious repeats before import even though Rally dedupes on email and domain.',
    ],
  ...
  airtable: {
    ...
    gotchas: [
      ...
      'Airtable automations are base-specific and do not transfer; rebuild the useful ones in Rally.',
    ],
  ...
  nutshell: {
    ...
    gotchas: [
      'Nutshell calls deals "leads"; map them to Rally deals during import.',
      'Nutshell email sequences do not transfer; rebuild active ones in Rally.',
  ...
  capsule: {
    ...
    gotchas: [
      'Capsule tags carry segmentation meaning; map key tags to Rally fields or segments.',
      'Capsule has light automation, so there is little to rebuild, but set up the Rally workflows you always wished you had.',
      'Confirm opportunity milestones map to your Rally pipeline stages.',
    ],
  ...
  streak: {
    ...
    gotchas: [
      ...
      'Boxes mix companies, contacts, and deals; split them into the right Rally objects on import.',
      'Streak has no shared automation to migrate; set up Rally workflows for the first time.',
    ],
  ...
  'less-annoying-crm': {
    ...
    gotchas: [
      'There are no automations to migrate, so plan the Rally workflows you never had before.',
      'Pipeline statuses are simple text; map them to Rally pipeline stages.',
      'Notes are stored per contact; confirm they land in the Rally notes field.',
    ],
  ...
  activecampaign: {
    ...
    kind: 'marketing',
    exportBody:
      'In ActiveCampaign, export Contacts and Deals to CSV, and export your automation list for reference. Automations are rebuilt in Rally, not imported.',
    gotchas: [
      'ActiveCampaign automations are the core of the tool and do not transfer; document active ones, then rebuild the sales-side sequences in Rally.',
      'Tags drive a lot of logic; map the meaningful ones to Rally fields or segments.',
      'Deal pipelines are secondary in ActiveCampaign; confirm stage mapping to Rally.',
    ],
  ...
  bitrix24: {
    ...
    gotchas: [
      ...
      'Bitrix24 business processes and automations do not transfer; rebuild the active sales ones in Rally.',
  ...
  odoo: {
    ...
    gotchas: [
      ...
      'Odoo automated actions and server actions do not transfer; rebuild the sales ones in Rally.',
  ...
  attio: {
    ...
    gotchas: [
      'Attio has a flexible object model; confirm each custom object maps to the right Rally object.',
      'Attio automations are newer and do not transfer; rebuild the active ones in Rally.',
  ...
  folk: {
    ...
    gotchas: [
      'folk is relationship-first, so deal data may be thin; set up your Rally pipeline fresh where needed.',
      'Groups map to Rally segments or fields rather than to a folder structure.',
  ...
  salesflare: {
    ...
    gotchas: [
      ...
      'Salesflare workflows do not transfer; rebuild active ones in Rally.',
      'Confirm opportunity stages map to your Rally pipeline.',
    ],
  ...
  apollo: {
    ...
    kind: 'engagement',
    exportBody:
      'In Apollo, export your saved Contacts, Accounts, and any deal data to CSV. Sequence content is rebuilt in Rally, and remember that Apollo prospecting data should be de-duplicated against your real book of business.',
    gotchas: [
      'Apollo mixes prospected data with your real accounts; import your qualified book, not the entire database.',
      'Apollo sequences do not transfer; rebuild the active ones as Rally sequences.',
  ...
  outreach: {
    ...
    kind: 'engagement',
    exportBody:
      'In Outreach, export Prospects and Accounts to CSV. Sequences are rebuilt in Rally, and if Outreach sat on top of a CRM, export the deal data from that system of record too.',
    gotchas: [
      'Outreach is an engagement layer, not a system of record; your deals likely live in a separate CRM you also need to export.',
      'Outreach sequences and rulesets do not transfer; rebuild the active ones as Rally sequences.',
      'Consolidating two systems (Outreach plus CRM) into Rally is the real work; plan the field mapping once.',
    ],
    timelineExtra: 'If you are collapsing Outreach plus a CRM into Rally, plan about one to two weeks; a lighter setup is faster.',
  },
  salesloft: {
    name: 'Salesloft',
    kind: 'engagement',
    exportBody:
      'In Salesloft, export People and Accounts to CSV. Cadences are rebuilt as Rally sequences, and export deal data from the underlying CRM if Salesloft was layered on one.',
    gotchas: [
      'Salesloft is a cadence layer, not a system of record; deals usually live in a separate CRM to export as well.',
      'Cadences and rulesets do not transfer; rebuild the active ones as Rally sequences.',
      'Combining Salesloft plus a CRM into one Rally workspace is the main effort; map fields once.',
    ],
    timelineExtra: 'Collapsing Salesloft plus a CRM into Rally takes about one to two weeks; a lighter migration is faster.',
  },
  engagebay: {
    name: 'EngageBay',
    kind: 'marketing',
    ...
    gotchas: [
      ...
      'EngageBay automations do not transfer; rebuild the active sales ones in Rally.',
      'Tags and lists map to Rally fields or segments rather than transferring as-is.',
    ],
  ...
  vtiger: {
    ...
    gotchas: [
      'Vtiger workflows and approval processes do not transfer; rebuild the active ones in Rally.',
  ...
  'sage-crm': {
    ...
    gotchas: [
      ...
      'Sage workflows and escalations do not transfer; rebuild the active sales ones in Rally.',
  ...
  'oracle-crm': {
    ...
    gotchas: [
      'Oracle CX is enterprise-heavy with complex object relationships; agree a clear mapping to Rally objects up front.',
      'Oracle process automation does not transfer; rebuild the active sales flows in Rally.',
      'Role and territory complexity does not map one to one, so plan Rally RBAC and views fresh.',
    ],
  ...
  sap: {
    ...
    gotchas: [
      ...
      'SAP workflow and business logic do not transfer; rebuild the sales-side flows in Rally.',
      'Standardize option and status values before mapping to Rally.',
    ],
  ...
  mailchimp: {
    ...
    kind: 'marketing',
    exportBody:
      'In Mailchimp, export your Audience (contacts) to CSV. Mailchimp has no real pipeline, so deals are set up fresh in Rally rather than imported.',
    gotchas: [
      'Mailchimp is email-first, so you are gaining a pipeline you never had; plan your Rally stages from scratch.',
      'Audience tags and merge fields map to Rally fields or segments.',
      'Campaign history stays in Mailchimp; export reports you want to keep for reference.',
    ],
    timelineExtra: 'Contacts import in a day; the setup time is designing the pipeline Rally gives you.',
  },
  'constant-contact': {
    name: 'Constant Contact',
    kind: 'marketing',
    exportBody:
      'In Constant Contact, export your contact lists to CSV. There is no sales pipeline to move, so deals start fresh in Rally.',
    gotchas: [
      'Constant Contact is an email tool, so you are adding true CRM structure; plan contacts, companies, and stages in Rally.',
      'Lists map to Rally segments or fields rather than transferring directly.',
      'Event and email history stays in Constant Contact; export what you need to keep.',
    ],
    timelineExtra: 'Contacts move in a day; the value comes from the CRM structure you build in Rally.',
  },
  'agile-crm': {
    name: 'Agile CRM',
    kind: 'crm',
    ...
    gotchas: [
      'Agile CRM campaigns and automations do not transfer; rebuild the active sales ones in Rally.',
      'Tags carry segmentation meaning; map the key ones to Rally fields or segments.',
      'Confirm deal milestones map to your Rally pipeline stages.',
    ],
  ...
  'act-crm': {
    ...
    gotchas: [
      ...
      'Act! has limited automation, so there is little to rebuild; set up the Rally workflows you always wanted.',
...
function buildSteps(s) {
  const dealBody =
    s.kind === 'sheet' || s.kind === 'marketing'
      ? `Bring in deal rows with their stage, value, close date, and owner if you track them. Map each stage to a Rally pipeline stage during import so nothing lands in the wrong column. If you did not track deals in ${s.name}, this is where Rally gives you a real pipeline for the first time.`
      : `Import open and recently won deals with their stage, value, close date, and owner. Map your ${s.name} stages to Rally pipeline stages during import so every deal lands in the right column, and include closed deals so your reporting history stays intact.`;

  const autoBody =
    s.kind === 'engagement'
      ? `Rebuild your ${s.name} cadences as Rally sequences. Engagement steps do not transfer between systems, so list your active cadences, then have Rook recreate them as Rally sequences with the same steps and timing. Most teams prune stale steps in the process.`
      : s.kind === 'marketing'
      ? `Recreate your ${s.name} campaigns and follow-up as Rally sequences and workflows. Automations do not transfer as files, so document the active ones, then have Rook rebuild the sales-side flows. Prune anything that was no longer firing.`
      : `Recreate your ${s.name} automations in Rally. Workflows and sequences do not transfer between systems, so list the active ones, then have Rook rebuild them as Rally workflows and sequences. Most teams take the chance to drop rules that were no longer firing.`;

  return [
    { h: `Export your data from ${s.name}`, body: s.exportBody },
    {
      h: 'Map your fields to Rally',
      body: `Line up your ${s.name} fields with Rally's objects: companies, contacts, deals, and any custom fields. Rally accepts a CSV per object and its importer suggests matches, so you mostly confirm rather than build a mapping from scratch. Decide now which fields are worth keeping and which legacy clutter to leave behind.`,
    },
    {
      h: 'Import accounts and contacts',
      body: `Import companies first, then contacts, so people link to the right accounts on the way in. Rally dedupes on email and domain and preserves record owners, so books of business stay intact rather than collapsing into one pile.`,
    },
    { h: 'Import deals and pipeline', body: dealBody },
    {
      h: 'Rebuild your pipeline, views, and dashboards with Rook',
      body: `This is where Rally saves the most time. Describe your process in plain language, for example "B2B pipeline with five stages, a manager forecast view, and a dashboard for pipeline by rep", and Rook builds the pipeline, saved views, and dashboards for you. No admin, no formula fields to hand-configure.`,
    },
    { h: 'Recreate automations and sequences', body: autoBody },
    {
      h: 'Validate, then go live',
      body: `Validate before you cut over. Spot-check record counts against ${s.name}, open a sample of deals to confirm stage and value, send a test from a sequence, and confirm dashboards total correctly. When it looks right, invite the team, set Rally as the system of record, and archive ${s.name}.`,
    },
  ];
}
...
export default Object.entries(MIG).map(([slug, s]) => {
  ...
  return {
    ...
    type: 'migration',
    title: `How to Migrate From ${s.name} to Rally (${YEAR})`,
    metaTitle: `Migrate From ${s.name} to Rally: Step-by-Step Guide (${YEAR}) | Rally`,
    metaDescription: `Move from ${s.name} to Rally in days, not months. Export, map fields, import records, and let Rook rebuild your pipeline and automations. Full step-by-step.`.slice(0, 155),
    eyebrow: `Migrate from ${s.name}`,
    h1: `How to migrate from ${s.name} to Rally`,
    shortAnswer: `Migrating from ${s.name} to Rally takes most teams a few days. Export your companies, contacts, and deals, map them to Rally's objects, then let Rook rebuild your pipeline, views, and automations from a plain-language description. Your records and history transfer, so you go live without a long re-implementation.`,
    intro: [
      `Switching from ${s.name} to Rally is a data export, a field mapping, and a Rook-assisted rebuild, not a multi-month re-implementation. This guide walks the full path: what to export, how fields map, how to bring in accounts, contacts, and deals, and how Rook rebuilds your pipeline and automations so you are live fast.`,
    ...
    stepsHeading: `Migrating from ${s.name} to Rally, step by step`,
    steps,
    sections: [
      {
        h: `What transfers from ${s.name}`,
        body: `Your core records and history come across. Here is what lands in Rally when you import cleanly:`,
        bullets: transfers,
      },
      {
    ...
    faqs: [
      {
        q: `How long does it take to migrate from ${s.name} to Rally?`,
      ...
      },
      {
        ...
        a: `No. Rally imports your companies, contacts, deals, and the activity history you export, including closed deals so reporting stays intact. Keep your ${s.name} export files as a backup, and run both systems in parallel until you have validated the data in Rally.`,
      },
      {
        q: `Do custom fields and automations transfer from ${s.name}?`,
        a: `Custom fields map directly to Rally custom fields during import. Automations do not transfer as files between any two systems, so you list your active ${s.name} workflows and Rook rebuilds them as Rally workflows and sequences. Most teams prune stale automations in the process.`,
      },
      {
      ...
      {
        q: `Can I run ${s.name} and Rally in parallel during the switch?`,
        a: `Yes, and you should. Import into Rally, validate record counts and a sample of deals, and keep ${s.name} read-only until the team is confident. Once dashboards total correctly and sequences send, set Rally as the system of record and archive ${s.name}.`,
      },
    ],
