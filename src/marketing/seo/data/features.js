...
function f(slug, title, kw, short, intro, vp, sec, faqs, table) {
  return {
    ...
    type: 'feature',
    title,
    metaTitle: `${title}: ${kw} (${YEAR}) | Ardovo`,
    ...
    valuePropsHeading: `How Ardovo delivers ${title.toLowerCase()}`,
    valueProps: vp.map((v, i) => ({ icon: ICONS[i % ICONS.length], h: v[0], body: v[1] })),
    ...(table
      ? {
          tableHeading: `${title} vs the old way`,
          table: { columns: ['Capability', 'Ardovo', 'Legacy tools'], rows: table },
          highlightCol: 1,
        }
      : {}),
...
const FEATURES = [
  f(
    ...
    [
      ['Live roll-up by rep and team', 'Ardovo rolls pipeline into a weighted and committed forecast the instant a deal changes stage or amount, with no spreadsheet export.'],
    ...
    [
      ...
      ['How Ardovo does forecasting', 'Ardovo builds the forecast from the same deal records reps already update, so there is nothing to reconcile. Rook narrates the call in plain language, points to the two or three deals that will decide the quarter, and drafts the follow-ups to de-risk them.'],
    ],
    [
      ['How accurate is Ardovo forecasting?', 'Accuracy comes from clean, current deal data. Because Ardovo forecasts from live records and Rook nudges reps to keep close dates and next steps current, the forecast reflects reality instead of quarter-end guesses.'],
      ...
      ['Do I need a separate BI tool?', 'No. Forecasting, dashboards, and revenue analytics are built into Ardovo at one price, so you do not need a separate BI or spreadsheet layer.'],
    ]
  ),
  f(
    ...
    'Lead scoring software ranks leads by how likely they are to buy, using fit signals (title, company size, industry) and engagement signals (opens, visits, replies). Ardovo scores every lead on arrival, so reps work the hottest ones first and Rook routes and follows up automatically instead of leads aging out.',
    'A lead score is only useful if it drives action. Ardovo connects the score to routing, sequences, and Rook so a hot lead gets worked in minutes, not days.',
    [
      ...
      ['Instant scoring on capture', 'New leads are scored the moment they hit Ardovo from a form, import, or integration, with no nightly batch delay.'],
    ...
    [
      ...
      ['How Ardovo does lead scoring', 'Ardovo scores on fit and engagement as leads arrive, routes the best ones to the right rep, and lets Rook open the conversation. As a lead engages more, the score climbs and Ardovo reprioritizes the queue automatically.'],
    ],
    [
      ['Does Ardovo score leads automatically?', 'Yes. Every lead is scored on capture and re-scored as engagement changes, with no manual tagging required.'],
      ...
      ['What happens to a high-scoring lead?', 'Ardovo can route it to the right owner and trigger a sequence, and Rook can draft and send the first touch so nothing hot goes cold.'],
    ]
  ),
  f(
    ...
    'Lead management software captures, organizes, routes, and nurtures leads from first touch to a qualified opportunity. Ardovo captures leads from forms and integrations, scores and routes them instantly, and lets Rook run first-touch outreach so no lead sits untouched and every source is measured end to end.',
    'Leads leak at the handoffs: capture, routing, first response, and nurture. Ardovo closes each gap so a lead moves forward the moment it arrives.',
    [
    ...
    [
      ...
      ['How Ardovo does lead management', 'Ardovo unifies capture, scoring, routing, and nurture in one place. Rook keeps first response fast, and conversion into a deal carries the full history so account executives start with context instead of a blank record.'],
    ],
    [
      ['How fast can Ardovo respond to a new lead?', 'Rook can respond within minutes of capture with a drafted, on-brand message, which is where most speed-to-lead advantage comes from.'],
      ['Can I track lead source ROI?', 'Yes. Every lead carries its source, so Ardovo reports conversion and revenue by channel without extra tagging.'],
  ...
  f(
    ...
    'Contact management software stores people, their details, and every interaction in one searchable place. Ardovo keeps a full contact record with roles, company links, and a complete activity timeline, and Rook enriches and de-duplicates automatically so your database stays clean without manual upkeep.',
    'A contact record is only as good as its context. Ardovo links every person to their company, deals, and full history so anyone can pick up the relationship instantly.',
    [
    ...
    [
      ...
      ['How Ardovo does contact management', 'Ardovo captures activity automatically, links people to companies and deals, and lets Rook enrich and de-dupe in the background so the record you open is complete and current.'],
    ],
    [
      ['Does Ardovo log emails and meetings automatically?', 'Yes. Connected email and calendar activity attaches to the right contact automatically, so the timeline builds itself.'],
      ['How does Ardovo handle duplicate contacts?', 'Rook detects likely duplicates and can merge them, preserving history so you keep one clean record per person.'],
  ...
  f(
    ...
    'Pipeline management software visualizes deals across stages so teams can move opportunities forward and spot where revenue stalls. Ardovo ships a visual pipeline that is alive on day one, with drag-to-advance stages, weighted values, and Rook watching for stuck deals and missing next steps.',
    'A pipeline should answer three questions instantly: what is here, what is stuck, and what to do next. Ardovo answers all three without a report build.',
    [
    ...
    [
      ...
      ['How Ardovo does pipeline management', 'Ardovo gives every team a live pipeline the day they start. Deals carry amount, stage, and next step, velocity is visible per stage, and Rook flags the deals that need attention before they slip.'],
    ],
    [
      ...
      ['How does Ardovo show stuck deals?', 'Ardovo tracks stage aging and missing next steps, and Rook surfaces the specific deals stalling so you can act.'],
  ...
  f(
    ...
    'Deal management software tracks each opportunity through to close with its value, stakeholders, and next steps in one record. Ardovo ships a deep deal object with line items, buying committee, competitors, and a full activity timeline, and Rook keeps next steps and close dates honest so deals do not drift.',
    'Shallow deal records lose the details that decide the outcome. Ardovo makes the deal the richest object in the CRM.',
    [
      ...
      ['Next-step enforcement', 'Every deal carries an owner and a next step, and Ardovo flags any deal missing one.'],
    ...
    [
      ...
      ['How Ardovo does deal management', 'Ardovo treats the deal as a first-class object with everything the outcome depends on. Rook watches for missing next steps and slipping dates and drafts the actions to keep each deal moving.'],
    ],
    [
      ...
      ['Does Ardovo track the buying committee?', 'Yes. Capture every stakeholder, their role, and their sentiment on the deal record.'],
      ['How does Ardovo prevent deals from stalling?', 'Ardovo requires a next step on every deal and Rook surfaces any that lack one or have a slipping close date.'],
    ]
  ),
  f(
    ...
    'Opportunity management software manages qualified deals from creation to close, tracking stage, value, and probability. Ardovo structures opportunities with stages, weighted amounts, and required next steps, and Rook monitors win-rate patterns and at-risk deals so managers coach on the deals that will move the number.',
    'Opportunity management is where forecasting, coaching, and execution meet. Ardovo keeps all three on one record.',
    [
    ...
    [
      ...
      ['How Ardovo does opportunity management', 'Ardovo links each opportunity to its contacts, company, and history, weights it by stage, and lets Rook surface the ones that need attention so time goes to the deals that matter.'],
    ],
    [
      ['What is the difference between a lead and an opportunity in Ardovo?', 'A lead is an unqualified contact or inquiry; an opportunity is a qualified deal with a value and stage. Ardovo converts one into the other in a click.'],
      ['Can I see win rates by rep?', 'Yes. Ardovo reports win rate by rep, stage, source, and product so you can coach precisely.'],
  ...
  f(
    ...
    'CPQ (configure, price, quote) software turns product selections into accurate, approved quotes fast. Ardovo builds quotes from your product catalog with line items, discounts, and approval rules, and Rook assembles a first-draft quote from the deal so reps stop rebuilding pricing in spreadsheets.',
    'CPQ removes the two things that slow quoting: pricing errors and approval delays. Ardovo handles both inside the deal.',
    [
      ...
      ['Quote to proposal to signature', 'Move from an approved quote to a branded proposal and e-signature without leaving Ardovo.'],
    ],
    [
      ...
      ['How Ardovo does CPQ', 'Ardovo keeps quoting inside the deal. The catalog drives pricing, approval rules keep discounts in bounds, and Rook drafts the first quote so reps send accurate pricing in minutes.'],
    ],
    [
      ['Does Ardovo handle discount approvals?', 'Yes. Set thresholds so standard discounts auto-approve and larger ones route to the right approver.'],
      ['Can quotes become proposals?', 'Yes. An approved quote flows into Ardovo Studio as a branded proposal and on to e-signature.'],
  ...
  f(
    ...
    'Proposal software helps teams create, send, and track branded sales proposals. Ardovo Studio builds proposals from reusable templates that pull deal and pricing data automatically, tracks when a prospect opens and reads each section, and lets Rook draft the first version so reps send in minutes, not hours.',
    'A proposal is a sales tool, not a document chore. Ardovo makes it fast to build and easy to learn from.',
    [
    ...
    [
      ...
      ['How Ardovo does proposals', 'Ardovo Studio turns the deal into a proposal in a click, tracks how the prospect engages, and hands off to e-signature, all in one price with the rest of the platform.'],
    ],
    [
      ['Can Ardovo track when a proposal is viewed?', 'Yes. Studio shows opens and which sections get the most attention so you can time follow-up.'],
      ...
      ['Can I send proposals for signature?', 'Yes. E-signature is built in, so a proposal goes from draft to signed inside Ardovo.'],
    ]
  ),
  f(
    ...
    'E-signature software lets you send documents for legally binding electronic signature and track their status. Ardovo includes e-signature on proposals and quotes with a full audit trail, reminders, and status on the deal, so contracts close inside the CRM instead of bouncing to a separate signing tool.',
    'The signature is the last mile of the deal. Ardovo keeps it attached to the record so status is never a mystery.',
    [
      ['Sign inside the deal', 'Send a quote or proposal for signature and watch status update on the deal without leaving Ardovo.'],
    ...
    [
      ...
      ['How Ardovo does e-signature', 'Ardovo sends proposals and quotes for signature from the deal, tracks status inline, and can advance the deal on signing, so the whole close happens in one system.'],
    ],
    [
      ['Are Ardovo e-signatures legally binding?', 'Yes. Signatures carry a timestamped audit trail consistent with common e-signature standards.'],
      ['Is there a per-envelope charge?', 'No. E-signature is included in Ardovo at one price, with no per-document metering.'],
  ...
  f(
    ...
    'Invoicing software creates, sends, and tracks invoices so you get paid on time. Ardovo generates invoices from closed deals and line items, tracks payment status, and lets Rook chase overdue balances, so billing flows straight from the sale instead of being re-entered in a separate accounting tool.',
    'When the deal already holds the line items, the invoice should write itself. Ardovo connects the sale to the money owed.',
    [
    ...
    [
      ...
      ['How Ardovo does invoicing', 'Ardovo turns a closed deal into an invoice with the correct line items, tracks payment status on the account, and lets Rook handle the follow-up so receivables do not slip.'],
    ],
    [
      ['Can Ardovo create invoices from deals?', 'Yes. A closed deal generates an invoice from its line items, so the amounts match the sale.'],
      ['Does Ardovo track overdue invoices?', 'Yes. Ardovo shows overdue balances and Rook can send escalating reminders automatically.'],
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does billing', 'Ardovo ties billing to the account and deal, tracks recurring and usage charges, and lets Rook catch failed payments and upcoming renewals so revenue does not leak.'],
    ],
    [
      ['Does Ardovo support recurring billing?', 'Yes. Ardovo handles subscriptions and usage-based charges tied to each account.'],
      ['How does Ardovo handle failed payments?', 'Ardovo flags failed charges and Rook can run dunning outreach to recover them.'],
  ...
  f(
    ...
    'Sales automation software removes repetitive selling tasks like data entry, follow-ups, and handoffs. Ardovo automates activity logging, task creation, routing, and reminders, and Rook executes multi-step work such as drafting emails and updating deals, so reps spend time selling instead of maintaining the CRM.',
    'Most sales automation stops at reminders. Ardovo goes further because Rook actually does the work, not just prompts you to.',
    [
      ['Automatic data capture', 'Emails, meetings, and calls log themselves to the right records, ending manual entry.'],
      ['Trigger-based workflows', 'When a deal changes stage or a lead is scored, Ardovo fires the right task, email, or handoff.'],
    ...
    [
      ...
      ['How Ardovo does sales automation', 'Ardovo captures activity automatically and fires workflows on real events. Where other tools stop at a reminder, Rook completes the task, so the automation produces finished work.'],
    ],
    [
  ...
  f(
    ...
    'Workflow automation software runs multi-step business processes automatically based on triggers and conditions. Ardovo includes a visual workflow builder where any event, from a new lead to a closed deal, can drive tasks, emails, field updates, and routing, and Rook can be a step that performs real work inside the flow.',
    'A workflow engine is only useful if non-engineers can build it. Ardovo makes automations visual and puts Rook inside them.',
    [
    ...
    [
      ...
      ['How Ardovo does workflow automation', 'Ardovo lets anyone build automations visually across every module, and Rook can be dropped in as a step that drafts, summarizes, or updates, so workflows produce finished output.'],
    ],
    [
      ['Do I need code to build workflows?', 'No. Ardovo workflows are built visually with triggers, conditions, and actions, no code required.'],
  ...
  f(
    ...
    'Marketing automation software nurtures leads with targeted campaigns and hands qualified ones to sales. Ardovo runs campaigns and sequences off the same contact data sales uses, so segmentation is accurate, handoffs are clean, and revenue is attributed end to end without stitching a separate marketing platform to the CRM.',
    'Marketing and sales fail at the seam between their tools. Ardovo removes the seam by sharing one database.',
    [
    ...
    [
      ...
      ['How Ardovo does marketing automation', 'Ardovo runs nurture and campaigns on the same database as sales, so segments are accurate, handoffs carry history, and attribution ties campaigns to closed revenue.'],
    ],
    [
      ['Does Ardovo replace a separate marketing platform?', 'For most teams, yes. Campaigns, sequences, and segmentation run natively on your CRM data at one price.'],
      ['Can I attribute revenue to campaigns?', 'Yes. Because leads convert to deals in the same system, Ardovo attributes closed revenue to source campaigns.'],
      ['How are qualified leads handed to sales?', 'Ardovo routes them to the right rep automatically with full campaign and activity history attached.'],
    ]
  ),
  f(
    ...
    'Email marketing software sends targeted email campaigns and measures how they perform. Ardovo sends campaigns to segments built from live CRM data, tracks opens, clicks, and replies against the contact record, and lets Rook draft and personalize copy, so email connects directly to pipeline instead of living in a silo.',
    'Email marketing works best when the list and the results live where deals live. Ardovo keeps both in the CRM.',
    [
    ...
    [
      ...
      ['How Ardovo does email marketing', 'Ardovo sends to CRM segments, records engagement on each contact, and lets Rook draft copy, so email feeds scoring, routing, and pipeline rather than sitting apart.'],
    ],
    [
  ...
  f(
    ...
    'Email tracking software tells you when a prospect opens your email, clicks a link, or replies. Ardovo tracks sent emails against the contact and deal, notifies the rep on opens and clicks, and lets Rook time follow-up around real engagement, so reps reach out when interest is highest instead of guessing.',
    'Knowing the moment a prospect engages changes when and how you follow up. Ardovo puts that signal on the record.',
    [
    ...
    [
      ...
      ['How Ardovo does email tracking', 'Ardovo tracks engagement on every sent email, attaches it to the record, and lets Rook act on spikes, so follow-up is timed to real interest.'],
    ],
    [
      ['Does Ardovo notify me when an email is opened?', 'Yes. You get real-time alerts on opens and link clicks for tracked emails.'],
  ...
  f(
    ...
    'Email sequence software sends a scheduled series of emails and tasks until a prospect replies or the sequence ends. Ardovo runs multi-step sequences that pause on reply, branch on engagement, and mix email with call and task steps, and Rook personalizes each send so cadences feel human instead of mass-blasted.',
    'A sequence should feel like a person, not a machine. Ardovo personalizes every step and stops the moment someone replies.',
    [
    ...
    [
      ...
      ['How Ardovo does sequences', 'Ardovo runs sequences that branch on engagement and pause on reply, with Rook personalizing each touch, so reps scale outreach without sounding automated.'],
    ],
    [
      ['Do sequences stop when someone replies?', 'Yes. Ardovo pauses a sequence the moment a prospect responds so follow-up never feels robotic.'],
  ...
  f(
    ...
    'Cold outreach software helps teams prospect new accounts at scale with personalized, compliant campaigns. Ardovo combines list building, sequences, and deliverability safeguards with Rook writing tailored openers, so cold outreach lands in inboxes and books meetings instead of getting flagged as spam or ignored.',
    'Cold outreach lives or dies on relevance and deliverability. Ardovo protects both while keeping volume high.',
    [
    ...
    [
      ...
      ['How Ardovo does cold outreach', 'Ardovo builds targeted lists, paces sends to protect deliverability, and lets Rook personalize openers, so campaigns book meetings rather than burn domains.'],
    ],
    [
      ['How does Ardovo protect email deliverability?', 'Ardovo paces sends and enforces limits that keep volume within healthy ranges for your domain.'],
  ...
  f(
    ...
    'Sales engagement software orchestrates every rep touch (email, call, social, task) across accounts and measures what works. Ardovo unifies engagement on the CRM record so every touch is logged in context, and Rook runs cadences and drafts messages, giving reps one place to execute outreach and managers one place to coach it.',
    'Engagement tools that sit beside the CRM create double entry. Ardovo makes the CRM the engagement layer.',
    [
    ...
    [
      ...
      ['How Ardovo does sales engagement', 'Ardovo runs engagement on the CRM record itself, so there is no separate tool to sync. Rook prioritizes and drafts, and managers coach from real activity and outcomes.'],
    ],
    [
      ['Is sales engagement separate from the CRM in Ardovo?', 'No. Engagement runs natively on the CRM record, so every touch is logged in context with no sync.'],
      ['Can Rook run a rep day?', 'Yes. Rook prioritizes the queue, drafts messages, and books meetings so reps execute faster.'],
      ['Can managers see engagement by rep?', 'Yes. Ardovo reports touches, replies, and outcomes per rep for precise coaching.'],
    ]
  ),
  f(
    ...
    [
      ...
      ['How Ardovo does dialing', 'Ardovo connects your phone system to the CRM so calls launch from records, log themselves, and feed insights, while Rook preps the call and drafts the follow-up.'],
    ],
    [
      ['Does Ardovo log calls automatically?', 'Yes. Calls log with duration, outcome, and notes attached to the contact and deal.'],
      ['Can I record and transcribe calls?', 'Yes. Recordings and transcripts feed conversation intelligence for coaching and search.'],
      ['Does Ardovo bring its own phone system?', 'Ardovo connects to phone providers like Aircall, RingCentral, and Dialpad so calling works from the record.'],
    ]
  ),
  f(
    ...
    'Meeting scheduling software lets prospects book time without the back-and-forth by sharing availability. Ardovo provides booking links and round-robin routing tied to the CRM, so a booked meeting creates or updates the right record, notifies the owner, and lets Rook prep an agenda, turning a calendar hold into pipeline context.',
    'A booked meeting should do more than block time. Ardovo makes it update the CRM and prep the rep.',
    [
    ...
    [
      ...
      ['How Ardovo does scheduling', 'Ardovo booking links and round-robin routing tie to the CRM, so every meeting updates the record, notifies the owner, and gets a Rook-prepared brief.'],
    ],
    [
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does calendar', 'Ardovo syncs your calendar both ways, links every meeting to the record, and lets Rook schedule and prep, so calendar time maps to pipeline.'],
    ],
    [
      ['Which calendars does Ardovo sync with?', 'Ardovo two-way syncs with Google Calendar and Outlook Calendar.'],
  ...
  f(
    ...
    'Sales reporting software turns CRM activity and pipeline into reports leaders use to run the business. Ardovo ships live reports on pipeline, activity, conversion, and win rates that update as records change, so you skip the weekly export-and-format ritual and Rook can answer reporting questions in plain language on demand.',
    'A report you rebuild every Monday is a tax. Ardovo makes reports live so the number is always current.',
    [
    ...
    [
      ...
      ['How Ardovo does sales reporting', 'Ardovo reports update live from CRM data and let you drill into records, while Rook answers ad hoc questions in plain language, so reporting stops eating your week.'],
    ],
    [
      ['Do I have to build reports by hand?', 'No. Ardovo ships live pipeline, activity, and conversion reports that update automatically.'],
  ...
  f(
    ...
    'Revenue reporting software tracks bookings, recognized revenue, and recurring revenue so finance and sales see one number. Ardovo reports revenue from the same deals, invoices, and subscriptions the team works, so bookings, ARR, and attainment tie out without spreadsheet reconciliation, and Rook can explain any movement on demand.',
    'When sales and finance report from different tools, the numbers never match. Ardovo reports revenue from one source.',
    [
    ...
    [
      ...
      ['How Ardovo does revenue reporting', 'Ardovo derives revenue reports from the deals, invoices, and subscriptions on the account, so sales and finance share one number, and Rook explains any change.'],
    ],
    [
      ['Does revenue reporting match finance?', 'Yes. Reports derive from the same records, so bookings, ARR, and attainment tie out.'],
      ['Can I see recurring revenue separately?', 'Yes. Ardovo separates new, recurring, and total revenue for a true picture.'],
  ...
  f(
    ...
    'Dashboard software displays key metrics in one live view so teams act on current numbers. Ardovo ships role-based dashboards for reps, managers, and leaders that are alive on day one, update as records change, and drill straight into the underlying deals, so everyone works from the same real-time picture instead of stale slides.',
    'A dashboard that is a day old drives yesterday decisions. Ardovo keeps every tile live and clickable.',
    [
    ...
    [
      ...
      ['How Ardovo does dashboards', 'Ardovo dashboards are live and role-based from day one, every tile drills to records, and Rook flags what moved, so the team acts on current reality.'],
    ],
    [
      ['Are Ardovo dashboards real-time?', 'Yes. Tiles update as records change, so you always see current numbers.'],
      ['Can I click into a dashboard metric?', 'Yes. Every KPI drills into the exact deals or activities behind it.'],
      ['Do dashboards work on day one?', 'Yes. Ardovo is alive with data immediately, so dashboards populate without a setup project.'],
    ]
  ),
  f(
    ...
    [
      ...
      ['How Ardovo does revenue analytics', 'Ardovo analyzes live pipeline, retention, and expansion, and Rook ties each trend to the accounts behind it, so analysis produces an action list, not just a dashboard.'],
    ],
    [
      ['Can Ardovo show revenue by cohort?', 'Yes. See how cohorts retain and expand over time to find durable growth.'],
      ['Does Ardovo flag churn risk?', 'Yes. Ardovo surfaces accounts at risk based on engagement and usage signals.'],
  ...
  f(
    ...
    'Sales analytics software measures how selling actually works: win rates, cycle time, stage conversion, and rep performance. Ardovo computes these from live deal data so you see where deals slow and which motions win, and Rook highlights the patterns worth coaching, turning raw activity into decisions instead of a wall of charts.',
    'Sales analytics is about finding the two changes that move the number. Ardovo surfaces them from live data.',
    [
    ...
    [
      ...
      ['How Ardovo does sales analytics', 'Ardovo computes win rate, cycle time, and stage conversion from live deals, and Rook surfaces the patterns most worth coaching, so analysis leads to action.'],
    ],
    [
      ['Can I see win rate by rep and stage?', 'Yes. Ardovo reports win rate, cycle time, and conversion by rep, stage, segment, and product.'],
      ['Where do deals slow down?', 'Ardovo shows stage conversion and drop-off so you fix the leakiest stage first.'],
      ['Does Ardovo surface what to coach?', 'Yes. Rook highlights the patterns and outliers most worth acting on.'],
    ]
  ),
  f(
    ...
    'Territory management software divides accounts among reps by geography, segment, or vertical and keeps coverage balanced. Ardovo assigns and rebalances territories with rules, routes new leads to the right owner automatically, and shows coverage gaps, so no account falls through the cracks and rebalancing does not mean a spreadsheet weekend.',
    'Territory design decides who owns what and how fairly. Ardovo keeps assignment and routing in sync automatically.',
    [
    ...
    [
      ...
      ['How Ardovo does territory management', 'Ardovo assigns and rebalances territories by rule, routes leads to the right owner automatically, and shows coverage gaps, so territory changes are fast and fair.'],
    ],
    [
      ['Can Ardovo route leads by territory?', 'Yes. New leads route automatically to the correct territory owner on arrival.'],
      ['How do I rebalance territories?', 'Adjust the rules and Ardovo reassigns accounts, preserving history, without a spreadsheet exercise.'],
      ['Can I see coverage gaps?', 'Yes. Ardovo shows uncovered accounts and overloaded reps so you balance with data.'],
    ]
  ),
  f(
    ...
    'Commission tracking software calculates what reps earn from closed deals against a comp plan. Ardovo computes commissions from the same deals it forecasts, applies your plan rules and accelerators, and gives reps a live view of earnings, so payouts are accurate, disputes drop, and finance is not rebuilding a comp spreadsheet each month.',
    'Comp disputes come from opaque math. Ardovo computes commissions from the deals reps already see.',
    [
    ...
    [
      ...
      ['How Ardovo does commission tracking', 'Ardovo computes commissions from closed deals using your plan rules and shows reps live earnings, so payouts are accurate and finance skips the monthly rebuild.'],
    ],
    [
      ['Can reps see their commissions live?', 'Yes. Reps see attainment and expected payout in real time, which cuts disputes.'],
      ['Does Ardovo support accelerators and tiers?', 'Yes. Apply tiered rates and accelerators from your comp plan automatically.'],
  ...
  f(
    ...
    'Sales performance management (SPM) software sets quotas and goals and tracks attainment across the team. Ardovo ties quotas, activity goals, and scorecards to live CRM data, so every rep and manager sees attainment and pacing in real time, and Rook flags who is off pace early enough to change the outcome.',
    'Performance management works when the scorecard is live, not a month-end surprise. Ardovo keeps it current.',
    [
    ...
    [
      ...
      ['How Ardovo does performance management', 'Ardovo ties quotas and scorecards to live CRM data so attainment is always current, and Rook flags who is off pace early enough to coach.'],
    ],
    [
      ['Can I set both revenue and activity goals?', 'Yes. Ardovo supports revenue and activity quotas by rep, team, and period.'],
      ['Is attainment real-time?', 'Yes. Attainment and pacing update as deals and activity change.'],
      ['Does Ardovo warn when a rep is off pace?', 'Yes. Rook surfaces reps drifting off pace early enough to change the outcome.'],
    ]
  ),
  f(
    ...
    'Sales coaching software helps managers improve reps using call reviews, scorecards, and data on what works. Ardovo combines conversation intelligence, deal analytics, and scorecards so managers coach from real calls and outcomes, and Rook flags coachable moments, turning coaching from a gut-feel 1:1 into a targeted, repeatable habit.',
    'Good coaching is specific and evidence-based. Ardovo puts the evidence in front of the manager.',
    [
    ...
    [
      ...
      ['How Ardovo does sales coaching', 'Ardovo gives managers call intelligence, deal analytics, and scorecards in one place, and Rook flags where coaching matters most, so development is targeted and measurable.'],
    ],
    [
      ['Can managers review calls in Ardovo?', 'Yes. Recorded calls and transcripts sit alongside the deal for context-rich review.'],
      ['Does Ardovo score rep behaviors?', 'Yes. Build scorecards on the behaviors that drive wins and track improvement over time.'],
      ['Can Ardovo show what to coach?', 'Yes. Rook flags the calls and deals where coaching would have the biggest impact.'],
    ]
  ),
  f(
    ...
    'Customer relationship management (CRM) software stores every customer, conversation, and deal in one place so teams sell and serve from shared context. Ardovo is an AI-native CRM that is alive with data on day one, unifies leads, contacts, companies, and deals, and puts Rook to work executing the follow-ups instead of just recording them.',
    'A CRM should reduce work, not create a second job of data entry. Ardovo is built so the record maintains itself.',
    [
      ...
      ['Alive on day one', 'Ardovo loads with working data and dashboards immediately, so there is no blank-database rollout.'],
    ...
    [
      ...
      ['How Ardovo does CRM', 'Ardovo unifies the whole revenue lifecycle in one system and lets Rook run the routine work, so reps spend time on customers rather than maintaining the database.'],
    ],
    [
      ['What makes Ardovo an AI-native CRM?', 'Ardovo is built around Rook, an operator that executes multi-step work like drafting emails, updating deals, and building quotes, rather than a chatbot bolted onto legacy CRM.'],
      ['How long does Ardovo take to set up?', 'Most teams are productive in minutes because Ardovo is alive with data on first load and Rook can configure pipelines and views from a sentence.'],
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does sales CRM', 'Ardovo centers on the deal and pipeline, forecasts from live data, and lets Rook handle the admin, so the team runs a tighter process with less overhead.'],
    ],
    [
      ['Is Ardovo good for a small sales team?', 'Yes. Ardovo is alive on day one and needs no admin to run, so small teams get enterprise-grade pipeline and forecasting without the overhead.'],
      ['Does Ardovo forecast automatically?', 'Yes. The forecast rolls up from live deals and updates as reps work, with Rook flagging at-risk deals.'],
  ...
  f(
    ...
    'AI CRM software uses artificial intelligence to prioritize work, draft communication, and automate updates. Ardovo is AI-native rather than AI-added: Rook, its operator, executes multi-step work like writing follow-ups, updating deals, building quotes, and answering questions from your data, so the CRM does work instead of only storing it.',
    'Most AI CRM features are a chat box beside the same old CRM. Ardovo makes the AI the operator that runs the work.',
    [
      ['Rook executes work', 'Rook completes tasks end to end, from drafting outreach to updating deals and building quotes.'],
      ['Prioritized every day', 'Ardovo ranks leads and deals so reps always know the highest-value next action.'],
    ...
    [
      ...
      ['How Ardovo does AI CRM', 'Ardovo is built around Rook, so the AI is the operator that runs routine revenue work, grounded in your live data and guarded by rules you control.'],
    ],
    [
      ['How is Ardovo different from a CRM with an AI add-on?', 'Ardovo is AI-native: Rook is the operator that executes work across the platform, not a chat widget bolted onto legacy CRM.'],
  ...
  f(
    ...
    'B2B CRM software manages complex, multi-stakeholder deals across accounts and long sales cycles. Ardovo models accounts, buying committees, and multi-threaded deals natively, tracks competitors and line items, and lets Rook keep every stakeholder warm, so B2B teams run committee-based deals without losing the thread between meetings.',
    'B2B deals are won across a committee over months. Ardovo is built to track every stakeholder and stage of that.',
    [
      ...
      ['Long-cycle awareness', 'Ardovo flags stalled stakeholders and slipping dates across long cycles.'],
    ...
    [
      ...
      ['How Ardovo does B2B CRM', 'Ardovo structures accounts and committees, keeps deal detail rich, and lets Rook maintain every relationship thread, so complex deals stay on track over long cycles.'],
    ],
    [
      ['Does Ardovo track the buying committee?', 'Yes. Every stakeholder, role, and sentiment is captured on the deal so multi-threaded selling is visible.'],
      ['Can Ardovo handle long sales cycles?', 'Yes. Ardovo flags stalled stakeholders and slipping dates so long-cycle deals do not drift.'],
      ['Is Ardovo account-based?', 'Yes. Accounts, contacts, and deals link together for a full account view.'],
    ],
    [
  ...
  f(
    ...
    'Small business CRM software gives lean teams pipeline, contacts, and follow-up without enterprise complexity or cost. Ardovo is alive on day one, needs no admin to run, and lets Rook handle the follow-ups a small team has no time for, so owners and small sales teams get a full revenue platform at one simple price.',
    'Small teams cannot afford a CRM that needs a specialist to run. Ardovo runs itself and Rook fills the gaps.',
    [
      ['No admin required', 'Ardovo is alive on day one and configures with a sentence, so no consultant is needed.'],
    ...
    [
      ...
      ['How Ardovo does small business CRM', 'Ardovo gives small teams a full revenue platform that runs itself, with Rook handling follow-up, so owners can sell without babysitting software.'],
    ],
    [
      ['Do I need someone to administer Ardovo?', 'No. Ardovo is alive on day one and configures from plain language, so no dedicated admin is required.'],
      ['Is Ardovo affordable for a small team?', 'Yes. One clean price includes every module, so there are no surprise upsells for core features.'],
      ['Can Ardovo grow with my business?', 'Yes. The same platform scales from a founder-led team to a full sales organization.'],
    ],
    [
  ...
  f(
    ...
    'Revenue operations software aligns sales, marketing, and success on shared data, process, and metrics. Ardovo gives RevOps one source of truth across the funnel, native forecasting, and workflow automation to enforce process, so RevOps teams stop stitching tools together and instead run a single system where the numbers already tie out.',
    'RevOps spends most of its time reconciling tools. Ardovo removes the reconciliation by unifying the data.',
    [
    ...
    [
      ...
      ['How Ardovo does RevOps', 'Ardovo unifies the funnel in one system so metrics agree, enforces process with workflows, and lets Rook automate the operational grind.'],
    ],
    [
      ['Does Ardovo give RevOps one source of truth?', 'Yes. Marketing, sales, and success data share one database, so metrics tie out without reconciliation.'],
      ['Can I enforce process in Ardovo?', 'Yes. Workflows enforce stage criteria, routing, and handoffs across teams.'],
  ...
  f(
    ...
    'Sales operations software gives ops teams the tools to design process, manage territories, and keep the sales engine running. Ardovo combines territory rules, workflow automation, forecasting, and clean reporting in one platform, so sales ops can set up the motion once and let Rook and workflows enforce it instead of chasing reps for updates.',
    'Sales ops should design the machine, not manually run it. Ardovo automates the enforcement.',
    [
      ['Territory and routing rules', 'Design territories and routing once and let Ardovo enforce ownership automatically.'],
    ...
    [
      ...
      ['How Ardovo does sales operations', 'Ardovo lets ops design territories and process once and enforces them with workflows and Rook, so the engine runs without constant manual intervention.'],
    ],
    [
      ['Can sales ops manage territories in Ardovo?', 'Yes. Design territories and routing by rule and Ardovo enforces ownership automatically.'],
      ['Does Ardovo reduce manual process policing?', 'Yes. Workflows and Rook enforce handoffs, next steps, and hygiene so ops does not chase reps.'],
  ...
  f(
    ...
    'Revenue intelligence software analyzes deals, activity, and conversations to predict outcomes and surface risk. Ardovo reads signals from live deals, emails, and calls to flag at-risk pipeline and forecast slippage early, and Rook explains why a deal is at risk and drafts the action to fix it, so intelligence turns into a next step.',
    'Revenue intelligence is only useful if it ends in an action. Ardovo connects the signal to the fix.',
    [
      ['Deal risk signals', 'Ardovo flags stalled activity, single-threading, and slipping dates before they cost the quarter.'],
    ...
    [
      ...
      ['How Ardovo does revenue intelligence', 'Ardovo reads live deal and activity signals to flag risk early, and Rook explains and acts on it, so intelligence produces a next step rather than a chart.'],
    ],
    [
      ['How does Ardovo detect deal risk?', 'Ardovo watches for stalled activity, single-threading, and slipping close dates and flags the specific deals at risk.'],
      ...
      ['Can Ardovo act on the insight?', 'Yes. Rook explains why a deal is at risk and drafts the outreach to fix it.'],
    ],
    [
  ...
  f(
    ...
    'Sales intelligence software gives reps data on accounts and contacts, including firmographics, enrichment, and buying signals. Ardovo enriches records automatically, surfaces intent and engagement signals, and lets Rook brief a rep on any account in seconds, so reps walk into every conversation informed instead of scrambling for context beforehand.',
    'Sales intelligence should arrive on the record, not in a separate tab. Ardovo enriches in place.',
    [
      ['Automatic enrichment', 'Ardovo fills firmographic and contact detail so records stay complete without manual research.'],
    ...
    [
      ...
      ['How Ardovo does sales intelligence', 'Ardovo enriches records in place, surfaces intent signals, and lets Rook brief reps on any account, so every conversation starts informed.'],
    ],
    [
      ['Does Ardovo enrich records automatically?', 'Yes. Ardovo fills firmographic and contact detail so records stay complete without manual research.'],
      ['Can Ardovo show buying signals?', 'Yes. Engagement and intent signals surface accounts worth prioritizing.'],
  ...
  f(
    ...
    'Conversation intelligence software records and analyzes sales calls to surface coaching moments and deal signals. Ardovo captures calls, transcribes them, and ties insights to the deal, so managers coach from real conversations and Rook can flag risk, next steps, and objections raised on the call, turning talk time into structured pipeline data.',
    'Every recorded call holds coaching gold and deal signals. Ardovo extracts both and attaches them to the deal.',
    [
    ...
    [
      ...
      ['How Ardovo does conversation intelligence', 'Ardovo records and transcribes calls, ties insight to the deal, and lets Rook summarize and draft follow-up, so conversations become structured pipeline data.'],
    ],
    [
      ['Does Ardovo transcribe sales calls?', 'Yes. Calls are recorded and transcribed automatically and attached to the deal.'],
      ['Can managers coach from calls?', 'Yes. Talk ratios, objections, and key moments surface for targeted coaching.'],
      ['Does Ardovo summarize calls?', 'Yes. Rook drafts a summary and follow-up so nothing from the call is lost.'],
    ],
    [
  ...
  f(
    ...
    'Relationship intelligence software maps who on your team knows whom at target accounts and how strong each connection is. Ardovo builds relationship maps from your communication history, surfaces the warmest path into an account, and lets Rook draft the intro request, so teams open doors through existing relationships instead of cold outreach.',
    'The warmest path into an account usually already exists on your team. Ardovo finds it and opens it.',
    [
    ...
    [
      ...
      ['How Ardovo does relationship intelligence', 'Ardovo maps team relationships from real activity, surfaces the warmest path into an account, and lets Rook draft the intro, so warm connections beat cold outreach.'],
    ],
    [
      ['Can Ardovo show who knows an account?', 'Yes. Ardovo maps which team members have a real connection to each account and how strong it is.'],
      ['How is connection strength determined?', 'Strength is inferred from actual communication history rather than a manual rating.'],
      ['Can Ardovo help request an intro?', 'Yes. Rook drafts the internal intro request so the warm path gets used.'],
    ],
    [
  ...
  f(
    ...
    'Sales enablement software gives reps the content, playbooks, and guidance to sell effectively. Ardovo surfaces the right collateral and next-best action inside the deal, keeps playbooks tied to stages, and lets Rook answer product and objection questions instantly, so reps get enablement in the flow of work instead of hunting through a separate portal.',
    'Enablement that lives in a separate portal goes unused. Ardovo puts it inside the deal where reps work.',
    [
    ...
    [
      ...
      ['How Ardovo does sales enablement', 'Ardovo surfaces content and playbooks inside the deal and lets Rook answer questions instantly, so reps get enablement in the flow of work.'],
    ],
    [
      ...
      ['Can I see which content works?', 'Yes. Ardovo shows which collateral actually advances deals so you invest in what works.'],
    ],
    [
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does sales onboarding', 'Ardovo makes the CRM alive and guided from day one and lets Rook answer questions and draft outreach, so new reps ramp in days.'],
    ],
    [
      ['How does Ardovo speed up ramp?', 'Ardovo is alive on day one with stage guidance and playbooks, and Rook answers questions and drafts outreach, so new hires get productive fast.'],
  ...
  f(
    ...
    'Lead generation software helps teams attract and capture new prospects and turn them into pipeline. Ardovo captures leads from forms and integrations, scores and routes them instantly, and lets Rook run first-touch outreach, so generated leads convert into real conversations instead of sitting in a list waiting for someone to notice them.',
    'Generating leads is wasted if they are not worked fast. Ardovo captures, scores, and works them instantly.',
    [
    ...
    [
      ...
      ['How Ardovo does lead generation', 'Ardovo captures leads from every source, scores and routes them instantly, and lets Rook work first touch, so generated leads become conversations fast.'],
    ],
    [
      ...
      ['Can I measure lead-source ROI?', 'Yes. Every lead carries its source, so Ardovo reports conversion and revenue by channel.'],
  ...
  f(
    ...
    'Lead routing software assigns incoming leads to the right rep instantly by rules like territory, round-robin, or score. Ardovo routes leads the moment they arrive, respects territory and availability, and notifies the owner in real time, so no lead waits in an unassigned queue and speed-to-lead stays measured in seconds, not hours.',
    'A lead sitting unassigned is a lead going cold. Ardovo routes and notifies the instant it arrives.',
    [
    ...
    [
      ...
      ['How Ardovo does lead routing', 'Ardovo routes leads by rule the moment they arrive, respects availability, and notifies the owner instantly, so speed-to-lead stays fast.'],
    ],
    [
      ['How fast does Ardovo route a lead?', 'Instantly. Leads route the moment they arrive, so they never sit in an unassigned queue.'],
  ...
  f(
    ...
    'Lead nurturing software keeps not-yet-ready leads warm with relevant, automated touches until they are ready to buy. Ardovo nurtures leads with multi-step campaigns off live CRM data, raises the lead score as engagement grows, and hands off to a rep when a lead heats up, so nurture flows straight into pipeline instead of a marketing dead end.',
    'Nurture should end in a warm handoff, not an endless drip. Ardovo hands off the moment a lead heats up.',
    [
    ...
    [
      ...
      ['How Ardovo does lead nurturing', 'Ardovo nurtures off live data, raises the score as leads engage, and hands off warm leads to a rep, so nurture feeds pipeline directly.'],
    ],
    [
      ['When does a nurtured lead go to sales?', 'When engagement pushes the score past your threshold, Ardovo routes it to a rep with full history.'],
  ...
  f(
    ...
    'Lead capture software collects prospect information from forms and channels and gets it into the CRM cleanly. Ardovo captures leads from web forms and integrations directly into the CRM, enriches and de-dupes them, and routes them instantly, so a captured lead becomes a scored, assigned, worked record without a manual import or a spreadsheet in between.',
    'A captured lead should land ready to work, not in a spreadsheet. Ardovo captures straight into pipeline.',
    [
      ['Direct-to-CRM capture', 'Web forms and integrations write leads straight into Ardovo with no import step.'],
    ...
    [
      ...
      ['How Ardovo does lead capture', 'Ardovo captures leads straight into the CRM, enriches and de-dupes them, and routes them instantly, so a captured lead is immediately workable.'],
    ],
    [
      ['Do captured leads need a manual import?', 'No. Web forms and integrations write straight into Ardovo, so there is no spreadsheet step.'],
  ...
  f(
    ...
    'Web form software builds forms that capture leads and feed them into your systems. Ardovo forms write submissions straight into the CRM as scored, routed leads, with enrichment and duplicate handling built in, so a form is not just a data collector but the front door to your pipeline, with Rook ready to run first touch on every submission.',
    'A web form should start a sales motion, not just email a notification. Ardovo makes the form pipeline-native.',
    [
      ['CRM-native forms', 'Form submissions become scored, routed CRM leads with no middleware.'],
      ['Built-in enrichment', 'Ardovo fills missing detail and de-dupes so form leads are clean.'],
    ...
    [
      ...
      ['How Ardovo does web forms', 'Ardovo forms feed the CRM directly as scored, routed leads, and Rook can run first touch, so a form submission starts a sales motion instantly.'],
    ],
    [
      ['Do Ardovo forms need middleware?', 'No. Submissions become scored, routed CRM leads directly, with no connector to maintain.'],
  ...
  f(
    ...
    'Sales prospecting software helps reps find and reach new potential customers efficiently. Ardovo combines list building from CRM and enrichment data, multi-channel sequences, and Rook writing tailored openers, so reps spend time on live conversations rather than building lists and rewriting the same cold email fifty times a day.',
    'Prospecting time should go to conversations, not list building. Ardovo automates the grind around it.',
    [
    ...
    [
      ...
      ['How Ardovo does prospecting', 'Ardovo builds targeted lists, runs multi-channel cadences, and lets Rook personalize openers, so reps spend time in live conversations.'],
    ],
    [
      ['Can Ardovo build prospect lists?', 'Yes. Build targeted lists from CRM data and enrichment to reach the right accounts.'],
      ['Does Ardovo personalize outreach?', 'Yes. Rook drafts tailored first lines from company and role context.'],
  ...
  f(
    ...
    'Outbound sales software powers proactive, rep-driven outreach at scale across email, phone, and social. Ardovo unifies list building, sequences, dialing, and follow-up on the CRM record, with Rook personalizing and pacing outreach, so outbound teams run high volume without losing personalization or letting activity go unlogged.',
    'Outbound at scale usually sacrifices personalization. Ardovo keeps both by letting Rook tailor every touch.',
    [
    ...
    [
      ...
      ['How Ardovo does outbound', 'Ardovo unifies the outbound stack on the CRM, lets Rook personalize at volume, and logs everything, so teams scale outreach without losing quality.'],
    ],
    [
      ...
      ['Does Ardovo protect deliverability?', 'Yes. Sending limits keep volume within healthy ranges for your domain.'],
    ],
    [
  ...
  f(
    ...
    'Inbound sales software captures interest from marketing and converts it fast before it cools. Ardovo captures inbound leads, scores and routes them in seconds, and lets Rook respond immediately with a relevant first touch, so inbound demand converts at its peak instead of aging in a queue while a competitor answers first.',
    'Inbound demand cools by the minute. Ardovo responds in seconds so interest converts at its peak.',
    [
    ...
    [
      ...
      ['How Ardovo does inbound sales', 'Ardovo captures and routes inbound leads in seconds and lets Rook respond immediately, so demand converts at its peak.'],
    ],
    [
      ['How fast does Ardovo respond to inbound?', 'Rook can respond within minutes, capturing interest at its peak before it cools.'],
      ['Do reps get context on inbound leads?', 'Yes. Leads arrive with source, campaign, and engagement history attached.'],
      ['Can I measure inbound conversion?', 'Yes. Ardovo reports conversion by source so marketing invests in what works.'],
    ],
    [
  ...
  f(
    ...
    'Sales cadence software structures a repeatable series of touches across channels so reps follow a proven outreach rhythm. Ardovo runs cadences that mix email, call, and task steps, pause on reply, and adapt by persona, with Rook personalizing each touch, so every rep follows the winning cadence without manually tracking who is due for what.',
    'A cadence keeps reps consistent when volume is high. Ardovo runs it and Rook keeps it personal.',
    [
    ...
    [
      ...
      ['How Ardovo does cadences', 'Ardovo runs multi-touch cadences that pause on reply and adapt by persona, with Rook personalizing each step, so reps stay consistent and human.'],
    ],
    [
  ...
  f(
    ...
    'Buyer intent software identifies which accounts are actively researching so teams reach them at the right moment. Ardovo surfaces intent from engagement signals across email, site, and activity, raises priority on accounts showing interest, and lets Rook draft timely outreach, so reps engage buyers when they are in-market rather than guessing who is ready.',
    'Reaching a buyer while they are researching changes everything. Ardovo surfaces that moment and acts on it.',
    [
    ...
    [
      ...
      ['How Ardovo does buyer intent', 'Ardovo surfaces intent from engagement, prioritizes interested accounts, and lets Rook draft timely outreach, so reps reach buyers in-market.'],
    ],
    [
      ['Where does Ardovo get intent signals?', 'From first-party engagement across email, site, and activity tied to your records.'],
      ['Does intent change prioritization?', 'Yes. Accounts showing intent rise in the queue automatically.'],
      ['Can Ardovo act on intent?', 'Yes. Rook drafts outreach tied to the signal so timing is right.'],
    ],
    [
  ...
  f(
    ...
    'Account management software gives teams a full view of each account, its contacts, deals, and history to grow the relationship. Ardovo links every contact, deal, activity, and invoice to the account, surfaces whitespace and expansion signals, and lets Rook prep account reviews, so account managers grow relationships from a complete picture, not scattered notes.',
    'Managing an account well requires seeing all of it at once. Ardovo puts the whole relationship on one record.',
    [
    ...
    [
      ...
      ['How Ardovo does account management', 'Ardovo links everything to the account, surfaces whitespace and health, and lets Rook prep reviews, so managers grow accounts from a complete picture.'],
    ],
    [
      ['Does Ardovo give a full account view?', 'Yes. Contacts, deals, activity, and billing link to the account for one complete picture.'],
      ['Can Ardovo show expansion opportunity?', 'Yes. Whitespace and engagement signals surface accounts ripe for growth.'],
  ...
  f(
    ...
    'Account planning software helps teams map stakeholders, set goals, and plan growth for strategic accounts. Ardovo captures the account plan alongside live data on the account, maps the buying committee and whitespace, and lets Rook keep the plan current and flag stalled stakeholders, so account plans stay living documents rather than a slide made once a year.',
    'Most account plans die in a slide deck. Ardovo keeps the plan attached to the live account and current.',
    [
    ...
    [
      ...
      ['How Ardovo does account planning', 'Ardovo keeps account plans on the live account with stakeholder maps and goals, and Rook keeps them current, so plans stay living documents.'],
    ],
    [
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does the contact database', 'Ardovo keeps a linked, searchable contact database that cleans itself through Rook, so records stay current without manual upkeep.'],
    ],
    [
      ['Does Ardovo keep contacts clean automatically?', 'Yes. Rook enriches, de-dupes, and flags stale records so the database stays trustworthy.'],
  ...
  f(
    ...
    'Sales tracking software records what reps do and how deals progress so managers see performance clearly. Ardovo tracks activity, pipeline movement, and outcomes automatically, so managers see who is doing what and where deals stand without asking, and Rook surfaces the reps and deals that need attention before a problem shows up in the number.',
    'Tracking should not mean nagging reps for updates. Ardovo tracks activity and pipeline automatically.',
    [
    ...
    [
      ...
      ['How Ardovo does sales tracking', 'Ardovo tracks activity and pipeline automatically and lets Rook flag what needs attention, so managers see performance without nagging reps.'],
    ],
    [
      ...
      ['Can managers see performance without meetings?', 'Yes. Ardovo shows rep and team performance live, so status meetings are not needed.'],
      ['Does Ardovo warn about problems early?', 'Yes. Rook surfaces the reps and deals that need attention before they hit the number.'],
    ],
    [
  ...
  f(
    ...
    'Activity tracking software records every rep interaction so the CRM reflects real work without manual entry. Ardovo captures emails, calls, and meetings automatically and attaches them to the right contact and deal, so the timeline builds itself and managers get accurate activity data, while Rook uses that activity to draft follow-ups and flag stalled deals.',
    'Manual activity logging is where CRM data goes to die. Ardovo captures it automatically instead.',
    [
    ...
    [
      ...
      ['How Ardovo does activity tracking', 'Ardovo captures activity automatically and attaches it to records, so the timeline builds itself and Rook acts on the signal.'],
    ],
    [
      ...
      ['Does Ardovo act on activity?', 'Yes. Rook drafts follow-ups and flags stalled deals from the activity signal.'],
    ],
    [
  ...
  f(
    ...
    'Sales call tracking software records and logs sales calls with outcomes so nothing is lost to memory. Ardovo logs every call to the right contact and deal with duration, outcome, and notes, records and transcribes for coaching, and lets Rook draft the follow-up, so call activity becomes structured pipeline data instead of scribbled notes that never make the CRM.',
    'A call that never makes the CRM never happened. Ardovo logs and learns from every one automatically.',
    [
    ...
    [
      ...
      ['How Ardovo does call tracking', 'Ardovo logs every call with outcome, records and transcribes it, and lets Rook draft follow-up, so calls become structured pipeline data.'],
    ],
    [
      ...
      ['Does Ardovo draft call follow-up?', 'Yes. Rook writes the post-call follow-up so momentum holds.'],
    ],
    [
  ...
  f(
    ...
    'Deal tracking software follows each opportunity through its stages with status, value, and next step visible. Ardovo tracks every deal on a live pipeline with a deep deal record, enforces a next step on each one, and lets Rook flag slipping dates and stalled deals, so a deal never goes dark and managers always know exactly where every opportunity stands.',
    'A tracked deal is one that never goes dark. Ardovo keeps status, value, and next step current on every one.',
    [
      ['Live deal status', 'See stage, value, and next step on every deal in real time.'],
      ['Next-step enforcement', 'Each deal carries a next step, and Ardovo flags any without one.'],
    ...
    [
      ...
      ['How Ardovo does deal tracking', 'Ardovo tracks every deal live with a next step and full history, and Rook flags slips, so no deal goes dark.'],
    ],
    [
      ['Does every deal show a next step?', 'Yes. Ardovo enforces a next step on each deal and flags any that lack one.'],
      ['Does Ardovo catch slipping deals?', 'Yes. Rook flags slipping close dates and stalled deals early.'],
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does the sales pipeline', 'Ardovo gives a live pipeline with coverage and velocity that feeds the forecast, and Rook keeps it honest, so the roll-up reflects reality.'],
    ],
    [
      ['Does the pipeline feed the forecast?', 'Yes. Pipeline value rolls into the weighted forecast automatically.'],
      ['Can I see coverage against quota?', 'Yes. Ardovo shows pipeline coverage and the gap to plan.'],
  ...
  f(
    ...
    'Pipeline forecasting software projects revenue from open pipeline using stage weighting and deal health. Ardovo forecasts from the live pipeline so weighted, commit, and best-case views update as deals move, shows coverage against quota, and lets Rook flag the deals putting the number at risk, so the forecast is a live picture rather than a quarter-end guess.',
    'Forecasting from pipeline works when the pipeline is honest and live. Ardovo keeps both true automatically.',
    [
    ...
    [
      ...
      ['How Ardovo does pipeline forecasting', 'Ardovo forecasts from the live pipeline with weighting and coverage, and Rook flags risk, so the forecast is a live picture, not a guess.'],
    ],
    [
      ['Does the forecast update as deals move?', 'Yes. Weighted, commit, and best-case views update in real time.'],
      ['Can I see coverage and the gap?', 'Yes. Ardovo shows coverage against quota and the exact dollar gap.'],
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does revenue forecasting', 'Ardovo projects new, recurring, and total revenue from one source that ties to reporting, and Rook explains change, so finance and sales share one number.'],
    ],
    [
      ['Does Ardovo forecast recurring revenue?', 'Yes. Renewals and subscriptions factor into the forecast automatically.'],
  ...
  f(
    ...
    'Sales forecasting tools help teams predict how much they will close by weighting pipeline and tracking attainment. Ardovo forecasts natively from live deals, so weighted, commit, and best-case roll-ups update in real time, coverage and gap-to-quota are always visible, and Rook flags at-risk deals, giving teams a forecast they can trust without a spreadsheet.',
    'The best forecasting tool is the one wired to your deals. Ardovo forecasts from live data, not a copy of it.',
    [
    ...
    [
      ...
      ['How Ardovo does forecasting tools', 'Ardovo forecasts from live deals with weighted roll-ups, quota tracking, and scenarios, and Rook flags risk, so the forecast is trustworthy without a spreadsheet.'],
    ],
    [
      ['Do Ardovo forecasting tools need a spreadsheet?', 'No. The forecast reads live deals, so there is nothing to export or reconcile.'],
  ...
  f(
    ...
    'Predictive sales software uses patterns in your data to predict which leads convert and which deals close. Ardovo scores leads and flags deal risk from signals in your own history, so predictions reflect how your business actually wins, and Rook turns each prediction into an action, so the insight ends in a drafted next step rather than a probability nobody uses.',
    'A prediction is only useful if it changes what a rep does next. Ardovo turns predictions into actions.',
    [
    ...
    [
      ...
      ['How Ardovo does predictive sales', 'Ardovo predicts lead conversion and deal risk from your own data, and Rook turns each prediction into a drafted next step, so insight becomes action.'],
    ],
    [
      ['What data drives Ardovo predictions?', 'Your own won and lost history, so predictions reflect how your business actually wins.'],
      ['Do predictions come with an action?', 'Yes. Rook drafts the next step, so a prediction is not just a number.'],
      ['Can it predict deal risk?', 'Yes. Ardovo predicts which deals are likely to slip or stall and why.'],
    ],
    [
  ...
  f(
    ...
    'AI sales assistant software helps reps by drafting outreach, prioritizing work, and answering questions. Ardovo gives every rep Rook, an operator that drafts and sends follow-ups, updates deals, builds quotes, and answers questions from live data, so the assistant executes real work rather than only suggesting it, and reps get hours back every week.',
    'Most AI assistants suggest; Rook executes. Ardovo gives every rep an operator that does the work.',
    [
    ...
    [
      ...
      ['How Ardovo does the AI sales assistant', 'Ardovo gives reps Rook, which drafts, prioritizes, answers, and executes multi-step work from live data, so the assistant produces finished output.'],
    ],
    [
  ...
  f(
    ...
    'Sales productivity software helps reps spend more time selling and less on admin. Ardovo removes manual logging with automatic activity capture, prioritizes the day so reps work the right deals, and lets Rook handle follow-ups and updates, so reps reclaim the hours usually lost to data entry and get a clear next action instead of a cluttered to-do list.',
    'Rep productivity is mostly about removing admin. Ardovo deletes the data entry and Rook does the busywork.',
    [
      ['No manual logging', 'Automatic activity capture removes the biggest admin drain.'],
      ['Prioritized queue', 'Ardovo ranks the day so reps work the highest-value action first.'],
    ...
    [
      ...
      ['How Ardovo does sales productivity', 'Ardovo removes logging, prioritizes the day, and lets Rook handle busywork on one platform, so reps get hours back for selling.'],
    ],
    [
      ['How does Ardovo save reps time?', 'By removing manual logging, prioritizing the day, and letting Rook handle follow-ups and updates.'],
      ['Does Ardovo reduce tool switching?', 'Yes. One platform covers pipeline, engagement, quotes, and reporting, so reps log in once.'],
  ...
  f(
    ...
    'Sales workflow software automates the steps of a sales process so nothing falls through the cracks. Ardovo lets teams build visual workflows that fire tasks, emails, routing, and field updates on real events, with Rook available as a step that produces finished work, so the sales process runs consistently without a manager policing every handoff.',
    'A sales process only holds if it runs itself. Ardovo automates the steps and Rook does the work in them.',
    [
    ...
    [
      ...
      ['How Ardovo does sales workflow', 'Ardovo lets anyone build visual workflows on real events, with Rook as a step that produces finished work, so the process runs consistently.'],
    ],
    [
      ['Do I need code to build workflows?', 'No. Ardovo workflows are visual, with triggers, conditions, and actions.'],
  ...
  f(
    ...
    'Sales team management software gives managers visibility into rep activity, pipeline, and performance to lead effectively. Ardovo shows live activity, pipeline, and attainment per rep, surfaces coaching moments from real calls and deals, and lets Rook flag who needs help, so managers coach from evidence and run the team without a wall of status meetings.',
    'Managing a team should run on evidence, not status meetings. Ardovo puts the evidence in front of managers.',
    [
    ...
    [
      ...
      ['How Ardovo does team management', 'Ardovo gives managers live per-rep visibility, coaching evidence, and Rook alerts, so they lead the team from data rather than meetings.'],
    ],
    [
      ['Can I see each rep performance live?', 'Yes. Activity, pipeline, and attainment update per rep in real time.'],
      ['Does Ardovo help with coaching?', 'Yes. Coach from real calls and deals, and Rook flags coachable moments.'],
  ...
  f(
    ...
    'Quota management software sets and tracks sales targets so every rep and team knows where they stand. Ardovo ties quotas to live CRM data, so attainment, pacing, and gap-to-goal update as deals close, and Rook flags who is off pace early, so quota tracking is a live scoreboard instead of a month-end spreadsheet nobody trusts.',
    'Quota tracking works when the scoreboard is live. Ardovo updates attainment as deals close, not at month end.',
    [
    ...
    [
      ...
      ['How Ardovo does quota management', 'Ardovo ties quotas to live data so attainment and gap-to-goal update as deals close, and Rook flags who is off pace.'],
    ],
    [
      ...
      ['Does Ardovo show the gap to goal?', 'Yes. See exactly how much is left and the pipeline needed to source it.'],
    ],
    [
  ...
  f(
    ...
    'Sales compensation software models comp plans and calculates what reps earn accurately. Ardovo computes commissions from the same deals it forecasts, applies plan rules, tiers, and accelerators, and shows reps live expected earnings, so payouts are accurate and transparent, disputes drop, and finance stops rebuilding a comp spreadsheet at the end of every period.',
    'Comp trust comes from transparent math on real deals. Ardovo computes it from the deals reps already see.',
    [
    ...
    [
      ...
      ['How Ardovo does sales compensation', 'Ardovo models comp plans and computes commissions from closed deals with live rep visibility, so payouts are accurate and disputes drop.'],
    ],
    [
      ['Does Ardovo support accelerators and tiers?', 'Yes. Model tiered rates and accelerators to match your plan.'],
  ...
  f(
    ...
    'Sales goal tracking software sets targets and tracks progress toward them across reps and teams. Ardovo ties revenue and activity goals to live CRM data, so progress and pacing update automatically, everyone sees the same scoreboard, and Rook flags goals at risk early, so goal tracking motivates the team instead of surprising them at the deadline.',
    'A goal you check once a month is a goal you miss. Ardovo keeps progress live and flags risk early.',
    [
    ...
    [
      ...
      ['How Ardovo does goal tracking', 'Ardovo ties goals to live data so progress updates automatically and Rook flags risk, so goals motivate instead of surprising.'],
    ],
    [
      ...
      ['Does Ardovo warn about missed goals?', 'Yes. Rook flags goals trending to a miss early enough to act.'],
    ],
    [
  ...
  f(
    ...
    'Sales KPI software tracks the metrics that show whether the sales engine is healthy: win rate, cycle time, activity, and attainment. Ardovo computes these live from CRM data on role-based dashboards, every tile drills into the records behind it, and Rook highlights which KPI moved and why, so leaders act on current numbers rather than stale slides.',
    'A KPI is only useful if it is live and you can act on it. Ardovo keeps every metric current and clickable.',
    [
    ...
    [
      ...
      ['How Ardovo does sales KPIs', 'Ardovo computes core KPIs live on role-based dashboards with drill-down, and Rook explains changes, so leaders act on current numbers.'],
    ],
    [
      ['Which KPIs does Ardovo track?', 'Win rate, cycle time, activity, coverage, attainment, and more, computed live from CRM data.'],
      ['Can I drill into a KPI?', 'Yes. Every tile drills into the exact records behind it.'],
      ['Does Ardovo explain KPI changes?', 'Yes. Rook highlights which metric moved and why.'],
    ],
    [
  ...
  f(
    ...
    'Call recording software captures sales and support calls with transcription for review, coaching, and compliance. Ardovo records connected calls, transcribes them, and attaches the recording to the contact and deal, so nothing said on a call is lost, reps skip manual notes, and managers coach from what actually happened.',
    'A call you cannot revisit is a call you learn nothing from. Ardovo keeps every call searchable on the record.',
    [
    ...
    [
      ...
      ['How Ardovo does call recording', 'Ardovo records and transcribes connected calls, attaches them to the record, and lets Rook summarize each one, so calls become searchable data and clean notes.'],
    ],
    [
      ['Does Ardovo transcribe calls?', 'Yes. Connected calls are transcribed and attached to the contact and deal.'],
      ['Can I search call transcripts?', 'Yes. Search across transcripts to find objections, competitors, and commitments.'],
      ['Does Ardovo summarize calls?', 'Yes. Rook writes a summary and next steps from each recorded call automatically.'],
    ]
  ),
  f(
    ...
    'Buying committee tracking software maps everyone involved in a purchase and their role, sentiment, and influence. Ardovo tracks the full committee on each deal, including champions, economic buyers, and blockers, so reps know who to engage next, and Rook flags single-threaded deals and missing stakeholders before they cost the deal.',
    'Deals die when they are single-threaded. Ardovo makes the whole committee visible so no stakeholder is missed.',
    [
      ...
      ['Single-thread warnings', 'Ardovo flags deals hanging on one contact so you multi-thread in time.'],
    ...
    [
      ...
      ['How Ardovo does buying committee tracking', 'Ardovo maps the full committee on each deal with role and sentiment, warns on single-threaded deals, and lets Rook recommend and draft the next stakeholder touch.'],
    ],
    [
      ['Can Ardovo track every stakeholder on a deal?', 'Yes. Capture each person, their role, sentiment, and influence on the deal record.'],
      ['Does Ardovo warn about single-threaded deals?', 'Yes. It flags deals hanging on one contact so you can multi-thread in time.'],
  ...
  f(
    ...
    'Sales content management software organizes decks, case studies, and collateral so reps always use the current, approved version. Ardovo keeps a central content library tied to deals and stages, tracks versions and usage, and lets Rook surface the right asset, so no rep sends an outdated deck or hunts through folders for a case study.',
    'The wrong version sent to a buyer erodes trust. Ardovo keeps one approved source and surfaces it in context.',
    [
    ...
    [
      ...
      ['How Ardovo does content management', 'Ardovo holds one approved library tied to deals and stages, controls versions, and lets Rook surface the right asset, so reps always send current, on-brand content.'],
    ],
    [
      ['How does Ardovo prevent outdated collateral?', 'Version control retires old files so only the current, approved version is available.'],
      ['Can I see which content reps use?', 'Yes. Ardovo tracks usage and prospect engagement across the library.'],
  ...
  f(
    ...
    'Document tracking software tells you when a shared document is opened and how it is read. Ardovo shares proposals, decks, and one-pagers as tracked links, showing opens, time per page, and forwards on the deal, so reps know when interest spikes and Rook can time follow-up to the moment a buyer is actively reading.',
    'Sending a document blind wastes your best timing signal. Ardovo shows exactly how buyers engage.',
    [
    ...
    [
      ...
      ['How Ardovo does document tracking', 'Ardovo shares documents as tracked links, records page-level engagement on the deal, and lets Rook time follow-up to the moment a buyer is reading.'],
    ],
    [
      ['Can Ardovo show which pages were read?', 'Yes. See page-level read time so you know what a buyer focused on.'],
      ['Does Ardovo detect forwards?', 'Yes. Forwarding alerts can reveal hidden committee members on the deal.'],
  ...
  f(
    ...
    'Data enrichment software fills in and updates missing contact and company details automatically. Ardovo enriches records on capture and on a schedule, adding titles, firmographics, and verified emails, and Rook flags stale or conflicting data, so your database stays accurate without reps manually researching and typing in fields.',
    'Sales runs on accurate data. Ardovo enriches records automatically so reps stop doing manual research.',
    [
    ...
    [
      ...
      ['How Ardovo does data enrichment', 'Ardovo enriches records on capture and on a schedule, verifies emails, and lets Rook flag stale data, so the database stays accurate without manual research.'],
    ],
    [
      ['Does Ardovo enrich new leads automatically?', 'Yes. New leads and contacts fill in firmographics and verified details on capture.'],
      ...
      ['Can Ardovo verify emails?', 'Yes. Email verification reduces bounces on outbound sends.'],
    ]
  ),
  f(
    ...
    'Landing page software builds focused web pages designed to convert visitors into leads. Ardovo landing pages capture directly into the CRM with built-in forms and tracking, so every conversion becomes a scored, attributed lead and Rook can follow up instantly, connecting campaign spend to pipeline without stitching separate page and CRM tools together.',
    'A landing page is only as good as what happens after the click. Ardovo connects conversions straight to pipeline.',
    [
    ...
    [
      ...
      ['How Ardovo does landing pages', 'Ardovo landing pages capture straight into the CRM with tracking, so conversions become attributed leads and Rook follows up fast, tying spend to pipeline.'],
    ],
    [
      ['Do landing page conversions go into the CRM?', 'Yes. Each conversion becomes a scored, attributed lead directly in Ardovo.'],
  ...
  f(
    ...
    'Chatbot software engages website visitors, answers questions, and qualifies leads automatically. Ardovo chat runs on your CRM data, so it can qualify, book meetings, and create leads in real time, and Rook powers responses that understand the account, turning your website into a 24/7 rep instead of a form that emails someone later.',
    'A chatbot should qualify and book, not just collect an email. Ardovo chat acts on the CRM in real time.',
    [
    ...
    [
      ...
      ['How Ardovo does chatbots', 'Ardovo chat qualifies, books, and creates leads in real time on your CRM data, with Rook powering account-aware answers and clean handoff to reps.'],
    ],
    [
  ...
  f(
    ...
    'Live chat software lets visitors and customers talk to your team in real time on your site. Ardovo live chat routes conversations to the right rep with full CRM context, logs the transcript to the record, and lets Rook triage and draft replies, so chat becomes a pipeline and support channel instead of a disconnected widget.',
    'Live chat is a sales channel when it knows who is on the other end. Ardovo gives every chat CRM context.',
    [
    ...
    [
      ...
      ['How Ardovo does live chat', 'Ardovo live chat gives reps full CRM context, routes by account and intent, logs transcripts to the record, and lets Rook triage, so chat drives pipeline and support.'],
    ],
    [
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does help desk', 'Ardovo manages tickets on the same customer record as sales and billing, tracks SLAs, and lets Rook triage and draft, so support resolves faster with full context.'],
    ],
    [
      ...
      ['Does Ardovo track SLAs?', 'Yes. Route tickets to queues and track response and resolution SLAs.'],
  ...
  f(
    ...
    'Ticketing software captures issues and requests as trackable tickets from open to resolved. Ardovo ticketing sits on the customer record with statuses, priorities, and SLAs, so every request is tracked with full account context, and Rook can categorize, route, and draft replies, keeping resolution fast and nothing slipping through the cracks.',
    'Untracked requests get forgotten. Ardovo turns every issue into a tracked ticket with account context.',
    [
    ...
    [
      ...
      ['How Ardovo does ticketing', 'Ardovo tracks every request as a ticket on the customer record with SLAs, and Rook categorizes, routes, and drafts, so resolution stays fast and accountable.'],
    ],
    [
  ...
  f(
    ...
    'Customer support software helps teams resolve customer issues across channels quickly and consistently. Ardovo handles support on the same record as sales and billing across email, chat, and tickets, so agents see the full relationship, and Rook triages and drafts, giving customers faster, more informed answers than a support tool cut off from the account.',
    'Great support depends on context. Ardovo gives agents the full customer relationship on every conversation.',
    [
    ...
    [
      ...
      ['How Ardovo does customer support', 'Ardovo resolves support across channels on the customer record, so agents work with full context and Rook speeds every response.'],
    ],
    [
      ['Is support connected to sales data?', 'Yes. Support runs on the same customer record as sales and billing.'],
      ['Does Ardovo support multiple channels?', 'Yes. Email, chat, and tickets all resolve on one record.'],
  ...
  f(
    ...
    [
      ...
      ['How Ardovo does customer success', 'Ardovo tracks health, renewals, and expansion on the customer record, and Rook flags risk and opportunity early, so CSMs protect and grow revenue proactively.'],
    ],
    [
      ['Does Ardovo score account health?', 'Yes. It combines usage, engagement, and support signals into a health score.'],
      ['Can I see renewals in advance?', 'Yes. Upcoming renewals surface early so nothing is a last-minute scramble.'],
      ['Does Ardovo flag churn risk?', 'Yes. Rook surfaces at-risk accounts and drafts the save play.'],
    ]
  ),
  f(
    ...
    [
      ...
      ['How Ardovo does customer onboarding', 'Ardovo runs onboarding as account-linked projects with milestones, and Rook keeps stalled steps moving, so customers reach value fast and renewals start strong.'],
    ],
    [
      ...
      ['Does Ardovo track time to value?', 'Yes. See how fast each customer hits key milestones.'],
    ]
  ),
  f(
    ...
    'Project management software plans and tracks work to deliver it on time. Ardovo includes projects tied to deals and accounts, so post-sale delivery, onboarding, and services run on the same platform as the revenue, with tasks, owners, and milestones visible to everyone, and Rook flagging slipping work before it affects the customer.',
    'When delivery lives apart from the deal, revenue and work drift out of sync. Ardovo keeps them together.',
    [
    ...
    [
      ...
      ['How Ardovo does project management', 'Ardovo runs delivery projects tied to deals and accounts, so work and revenue stay in sync, and Rook flags slippage before it reaches the customer.'],
    ],
    [
      ...
      ['Does Ardovo warn about slipping work?', 'Yes. Rook surfaces slipping tasks and milestones early.'],
    ]
  ),
  f(
    ...
    'Task management software organizes what needs doing so nothing falls through the cracks. Ardovo attaches tasks to leads, deals, and accounts with due dates and owners, surfaces them in a prioritized daily list, and lets Rook create and complete routine tasks, so reps spend the day on the right work instead of a scattered to-do list.',
    'A to-do list detached from the deal is just noise. Ardovo ties every task to the record it moves forward.',
    [
    ...
    [
      ...
      ['How Ardovo does task management', 'Ardovo attaches tasks to the records they move, ranks them for the day, and lets Rook handle routine ones, so reps focus on work that matters.'],
    ],
    [
      ['Are tasks connected to deals?', 'Yes. Attach tasks to leads, deals, and accounts with owners and due dates.'],
      ['Does Ardovo prioritize my tasks?', 'Yes. Reps get a prioritized daily list of the right next actions.'],
  ...
  f(
    ...
    'Sales tax software calculates the right tax on each sale and supports compliance. Ardovo applies tax to quotes and invoices based on product and location, so amounts are correct at quote time and flow into billing, keeping the numbers accurate end to end instead of bolting a tax tool onto a disconnected invoicing step.',
    'Tax errors surface at invoicing when they are expensive to fix. Ardovo gets it right at quote time.',
    [
    ...
    [
      ...
      ['How Ardovo does sales tax', 'Ardovo applies tax on quotes and invoices by product and location, so amounts are right from quote to invoice and sync cleanly to accounting.'],
    ],
    [
      ['Does Ardovo calculate tax on quotes?', 'Yes. Tax applies by product and location as quotes are built.'],
