...
.jug-progress-bar { display: block; height: 100%; width: 0%; background: var(--m-grad, linear-gradient(100deg,#0e9f8f,#14b8a6 46%,#7c5cf7)); box-shadow: 0 0 10px rgba(14,159,143,.5); transition: width .08s linear; }

.jug-breadcrumb { font-size: 14px; color: var(--m-ink3, #7c8399); margin: 30px 0 22px; }
.jug-breadcrumb a { color: var(--m-ink3, #7c8399); }
.jug-breadcrumb a:hover { color: var(--m-accent, #0e9f8f); }
...
.jug-aurora span:nth-child(1) { width: 420px; height: 420px; left: -60px; top: -120px; background: radial-gradient(circle, rgba(14,159,143,.55), transparent 68%); animation: jugAurora 14s ease-in-out infinite; }
.jug-aurora span:nth-child(2) { width: 360px; height: 360px; right: 0; top: -80px; background: radial-gradient(circle, rgba(20,184,166,.42), transparent 66%); animation: jugAurora 18s ease-in-out infinite reverse; }
...
.jug-eyebrow { display: inline-block; font-size: 13px; font-weight: 800; letter-spacing: .16em; text-transform: uppercase; color: var(--m-accent, #0e9f8f); margin-bottom: 12px; margin-right: 8px; }
...
.jug-hero-stat:hover { transform: translateY(-3px); box-shadow: 0 18px 40px -22px rgba(14,159,143,.5); border-color: color-mix(in srgb, var(--m-accent, #0e9f8f) 45%, var(--m-line, #eceef4)); }
.jug-hero-stat-value { font-size: clamp(1.9rem, 3vw, 2.7rem); font-weight: 800; letter-spacing: -.03em; line-height: 1; background: var(--m-grad, linear-gradient(100deg,#0e9f8f,#14b8a6 46%,#7c5cf7)); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; color: transparent; font-variant-numeric: tabular-nums; }
...
.jug-toc li.active { border-left-color: var(--m-accent, #0e9f8f); }
.jug-toc li.active a { color: var(--m-accent, #0e9f8f); font-weight: 700; }
...
.jug-stat:hover { transform: translateY(-4px); box-shadow: 0 20px 44px -24px rgba(14,159,143,.5); border-color: color-mix(in srgb, var(--m-accent, #0e9f8f) 40%, var(--m-line, #eceef4)); }
...
.jug-trend-up { color: var(--m-teal, #0e9f8f); }
...
.jug-calc-shell { display: grid; grid-template-columns: 1fr 1fr; gap: 22px; align-items: start; border: 1px solid var(--m-line, #eceef4); border-radius: 20px; padding: 24px; background: linear-gradient(140deg, color-mix(in srgb, var(--m-accent,#0e9f8f) 6%, var(--m-bg2, #f7f8fc)), var(--m-bg2, #f7f8fc)); }
...
.jug-calc-field input:focus, .jug-calc-field select:focus { outline: 2px solid var(--m-accent, #0e9f8f); outline-offset: 1px; border-color: var(--m-accent, #0e9f8f); }
.jug-calc-range { display: flex; align-items: center; gap: 12px; }
.jug-calc-range input[type="range"] { flex: 1; accent-color: var(--m-accent, #0e9f8f); }
.jug-calc-range output { font-size: 15px; font-weight: 700; color: var(--m-accent, #0e9f8f); min-width: 56px; text-align: right; }
...
.jug-calc-outputs tr.is-key { background: linear-gradient(90deg, color-mix(in srgb, var(--m-accent,#0e9f8f) 10%, transparent), transparent); }
.jug-calc-outputs tr.is-key td { color: var(--m-accent, #0e9f8f); font-size: 19px; }
...
.jug-matrix tbody tr:hover { background: color-mix(in srgb, var(--m-accent,#0e9f8f) 4%, transparent); }
.jug-matrix .is-key { background: linear-gradient(180deg, rgba(14,159,143,.1), rgba(14,159,143,.03)); }
.jug-matrix thead .is-key { color: var(--m-accent, #0e9f8f); background: rgba(14,159,143,.08); }
.jug-cell-yes { color: var(--m-teal, #0e9f8f); font-weight: 800; }
...
.jug-pc-pro { border-top: 3px solid var(--m-teal, #0e9f8f); }
...
.jug-pc-pro .jug-pc-head { color: var(--m-teal, #0e9f8f); }
...
.jug-pc-pro li::before { content: ""; position: absolute; left: 0; top: 7px; width: 13px; height: 8px; border-left: 2.5px solid var(--m-teal, #0e9f8f); border-bottom: 2.5px solid var(--m-teal, #0e9f8f); transform: rotate(-45deg); }
...
ol.jug-steps > .jug-step::before { content: counter(jugstep); position: absolute; left: 0; top: 0; width: 42px; height: 42px; border-radius: 13px; background: var(--m-grad, linear-gradient(100deg,#0e9f8f,#14b8a6 46%,#7c5cf7)); color: #fff; font-weight: 800; display: grid; place-items: center; font-size: 17px; box-shadow: 0 10px 22px -10px rgba(14,159,143,.7); }
ol.jug-steps > .jug-step:not(:last-child)::after { content: ""; position: absolute; left: 20px; top: 46px; bottom: 4px; width: 2px; background: linear-gradient(var(--m-accent,#0e9f8f), transparent); }
ul.jug-steps > .jug-step { padding: 4px 0 16px 24px; position: relative; }
ul.jug-steps > .jug-step::before { content: ""; position: absolute; left: 0; top: 9px; width: 10px; height: 10px; border-radius: 50%; background: var(--m-accent, #0e9f8f); }
...
.jug-quote { margin: 0 0 42px; border-left: 4px solid var(--m-accent, #0e9f8f); padding: 8px 0 8px 26px; position: relative; }
...
.jug-callout::before { content: ""; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: var(--m-grad, linear-gradient(100deg,#0e9f8f,#14b8a6 46%,#7c5cf7)); }
...
.jug-tone-accent { background: linear-gradient(120deg, rgba(14,159,143,.09), rgba(20,184,166,.05)); border-color: rgba(14,159,143,.22); }
...
.jug-faq-item[open] { border-color: color-mix(in srgb, var(--m-accent,#0e9f8f) 35%, var(--m-line, #eceef4)); box-shadow: 0 12px 30px -22px rgba(14,159,143,.5); }
...
.jug-faq-item summary::after { content: "+"; position: absolute; right: 2px; top: 12px; font-size: 24px; color: var(--m-accent, #0e9f8f); font-weight: 300; transition: transform .2s ease; }
...
.jug-related li a { display: inline-flex; align-items: center; gap: 8px; color: var(--m-accent, #0e9f8f); font-weight: 700; font-size: 16px; }
.jug-related li a::before { content: "->"; opacity: .6; font-weight: 400; }

.jug-cta { border: 1px solid rgba(14,159,143,.22); border-radius: 24px; padding: 40px 32px; text-align: center; background: linear-gradient(135deg, rgba(14,159,143,.1), rgba(20,184,166,.06) 55%, rgba(14,159,154,.06)); position: relative; overflow: hidden; }
.jug-cta .jug-h { font-size: clamp(1.7rem, 3vw, 2.3rem); }
.jug-cta-btn { display: inline-block; background: var(--m-grad, linear-gradient(100deg,#0e9f8f,#14b8a6 46%,#7c5cf7)); background-size: 200% auto; color: #fff; font-weight: 700; padding: 14px 30px; border-radius: 13px; font-size: 16px; box-shadow: 0 14px 34px -14px rgba(14,159,143,.75); transition: transform .18s ease, background-position .5s ease; }
.jug-cta-btn:hover { transform: translateY(-2px); background-position: 100% 0; }
.jug-cta-link { display: inline-block; margin-left: 14px; color: var(--m-accent, #0e9f8f); font-weight: 700; padding: 14px 4px; }
`;
