...
// parentheses, text concat with &), dependency-ordered recompute
// with cycle detection, per-cell formatting, and a deterministic
...
// "function" declaration or a class declared above that call. No
// const arrow is invoked during seeding.
//
// Every writer carries a // SUPABASE: note for the live swap
// (tables namespaced rally_sheet_*).
// ============================================================
import { useEffect, useState } from 'react';

const LS_KEY = 'rally_sheets_v1'; // bump to force a clean reseed

/* Supported functions, surfaced in the in-app formula help. */
...
/* ============================================================
   FORMULA ERROR  (declared before any runtime use; seeding never
   references it, so no TDZ concern)
   ============================================================ */
...
/* ============================================================
   A1 ADDRESS HELPERS  (hoisted function declarations)
   ============================================================ */
...
export function colLabel(n) { return numToCol(n); }
...
/* ============================================================
   TOKENIZER + RECURSIVE-DESCENT PARSER  (hoisted)
   Grammar (loosest to tightest binding):
     compare -> concat -> add -> mul -> pow -> unary -> primary
   ============================================================ */
function tokenize(src) {
  ...
  while (i < src.length) {
    ...
    if (isDigit(c) || (c === '.' && isDigit(src[i + 1]))) {
      let j = i, num = '';
      while (j < src.length && (isDigit(src[j]) || src[j] === '.')) { num += src[j]; j++; }
      toks.push({ t: 'num', v: parseFloat(num) }); i = j; continue;
    }
    if (isAlpha(c)) {
      let j = i, id = '';
      while (j < src.length && (isAlpha(src[j]) || isDigit(src[j]) || src[j] === '_')) { id += src[j]; j++; }
      toks.push({ t: 'id', v: id }); i = j; continue;
    }
...
function parseFormula(toks) {
  ...
  const peek = () => toks[p];
  const next = () => toks[p++];
  const isOp = (v) => { const t = peek(); return t && t.t === 'op' && (v == null || (Array.isArray(v) ? v.indexOf(t.v) !== -1 : t.v === v)); };
  const expect = (v) => { const t = next(); if (!t || t.v !== v) throw new FormulaError('#ERR!'); };

  function parseExpr() { return parseCompare(); }
  function parseCompare() {
    let left = parseConcat();
    while (isOp(['=', '<', '>', '<=', '>=', '<>'])) { const op = next().v; left = { k: 'bin', op, left, right: parseConcat() }; }
    return left;
  }
  function parseConcat() {
    let left = parseAdd();
    while (isOp('&')) { next(); left = { k: 'bin', op: '&', left, right: parseAdd() }; }
    return left;
  }
  function parseAdd() {
    let left = parseMul();
    while (isOp(['+', '-'])) { const op = next().v; left = { k: 'bin', op, left, right: parseMul() }; }
    return left;
  }
  function parseMul() {
    let left = parsePow();
    while (isOp(['*', '/'])) { const op = next().v; left = { k: 'bin', op, left, right: parsePow() }; }
    return left;
  }
  function parsePow() {
    let left = parsePostfix();
    while (isOp('^')) { next(); left = { k: 'bin', op: '^', left, right: parsePostfix() }; }
    return left;
  }
  function parsePostfix() {
    let node = parseUnary();
    while (isOp('%')) { next(); node = { k: 'unary', op: '%', arg: node }; }
    return node;
  }
  function parseUnary() {
    if (isOp(['-', '+'])) { const op = next().v; return { k: 'unary', op, arg: parseUnary() }; }
    return parsePrimary();
  }
  function parsePrimary() {
    ...
    if (t.t === 'id') {
      ...
      if (isOp(':')) { next(); const t2 = next(); if (!t2 || t2.t !== 'id') throw new FormulaError('#ERR!'); return { k: 'range', from: name.toUpperCase(), to: t2.v.toUpperCase() }; }
...
/* ============================================================
   EVALUATOR  (hoisted). cv(id) resolves a cell's computed value.
   ============================================================ */
function nodeValues(node, cv) {
  // Flatten ranges to a scalar list; everything else is a single value.
...
function evalAst(node, cv) {
  switch (node.k) {
    ...
    case 'ref': return cv(node.id);
    case 'range': throw new FormulaError('#VALUE!'); // a bare range is not a scalar
    case 'unary': {
      if (node.op === '%') return toNum(evalAst(node.arg, cv)) / 100;
      const v = toNum(evalAst(node.arg, cv));
      return node.op === '-' ? -v : v;
    }
    case 'bin': return evalBin(node, cv);
    case 'func': return evalFunc(node, cv);
    default: throw new FormulaError('#ERR!');
  }
}
...
function evalFunc(node, cv) {
  ...
  switch (node.name) {
    ...
    case 'AVERAGE': { const n = nums(); if (!n.length) throw new FormulaError('#DIV/0!'); return n.reduce((s, x) => s + x, 0) / n.length; }
    case 'MIN': { const n = nums(); return n.length ? Math.min(...n) : 0; }
    case 'MAX': { const n = nums(); return n.length ? Math.max(...n) : 0; }
    case 'COUNT': return nums().length;
    case 'COUNTA': return gather().filter((v) => v !== '' && v != null).length;
    case 'ROUND': { const x = toNum(arg(0)); const d = node.args.length > 1 ? toNum(arg(1)) : 0; const f = Math.pow(10, d); return Math.round(x * f) / f; }
    case 'ABS': return Math.abs(toNum(arg(0)));
    case 'IF': { const c = truthy(arg(0)); if (c) return arg(1); return node.args.length > 2 ? arg(2) : false; }
    case 'CONCAT': case 'CONCATENATE': return gather().map(toStr).join('');
    default: throw new FormulaError('#NAME?');
  }
}

/* Evaluate one raw cell string to a scalar value (number | string | boolean). */
...
/* ============================================================
   RECOMPUTE  (hoisted). Dependency-ordered, memoized, with a
   visiting set for cycle detection. Returns { id: {value, error} }.
   ============================================================ */
export function computeSheet(sheet) {
  const cache = new Map();
  const visiting = new Set();
  const rawOf = (id) => { const c = sheet.cells[id]; return c ? c.raw : ''; };
  function cellValue(id) {
    if (cache.has(id)) return cache.get(id);
    if (visiting.has(id)) throw new FormulaError('#CYCLE!');
    visiting.add(id);
    let val;
    try { val = evalRaw(rawOf(id), cellValue); }
  ...
  for (const id of Object.keys(sheet.cells)) {
    ...
    try { out[id] = { value: cellValue(id), error: null }; }
    catch (e) { out[id] = { value: null, error: e && e.code ? e.code : '#ERR!' }; }
  }
  return out;
}

/* Format a computed value for display given a per-cell number format. */
export function formatCellValue(value, fmt) {
...
/* ============================================================
   SEED  (hoisted). Deterministic - fixed content, no Date.now /
   Math.random. Builds three workbooks whose formulas compute.
   ============================================================ */
function blankSheet(id, name, rows = 50, cols = 26) { return { id, name, rows, cols, cells: {} }; }

function cellsFrom(grid) {
  // grid: array of rows; each row is an array of specs. A spec is a string /
  // number (raw only) or an object { raw, fmt, bold, align, bg }. null skips.
  const cells = {};
  grid.forEach((row, ri) => {
    (row || []).forEach((spec, ci) => {
      if (spec == null) return;
...
function buildSeed() {
  const H = (raw) => ({ raw, bold: true });          // bold header
  const CUR = (raw) => ({ raw, fmt: 'currency' });    // currency
  const PCT = (raw) => ({ raw, fmt: 'percent' });     // percent
  const NUM = (raw) => ({ raw, fmt: 'number' });      // thousands
...
/* ============================================================
   PERSISTENCE + PUB/SUB  (state seeds here; all helpers above are
   hoisted function declarations, so no TDZ)
   ============================================================ */
let state = load();
const subs = new Set();

function load() {
  try { const raw = localStorage.getItem(LS_KEY); if (raw) { const s = JSON.parse(raw); if (s && Array.isArray(s.workbooks)) return s; } } catch {}
...
export function resetSheets() { try { localStorage.removeItem(LS_KEY); } catch {} state = load(); subs.forEach((fn) => fn(state)); }
export function getSheetsState() { return state; }

export function useSheets(selector = (s) => s) {
...
let idc = 1;
const nid = (p) => `${p}_${Date.now().toString(36)}${(idc++).toString(36)}`;

/* ============================================================
   READ API
   ============================================================ */
export const getWorkbooks = () => state.workbooks;                                   // SUPABASE: from('rally_sheet_workbooks').select()
export const getWorkbook = (id) => state.workbooks.find((w) => w.id === id);
export const getSheet = (wbId, sheetId) => { const w = getWorkbook(wbId); return w ? w.sheets.find((s) => s.id === sheetId) : null; };

/* Small stat used by the workbook picker cards: how many filled + formula cells. */
export function workbookStats(wb) {
  let filled = 0, formulas = 0;
  for (const sh of wb.sheets) {
    for (const id of Object.keys(sh.cells)) {
      const raw = sh.cells[id].raw;
      if (raw != null && raw !== '') { filled++; if (String(raw)[0] === '=') formulas++; }
    }
  }
...
/* ============================================================
   WRITE API   (validated writers; each notes the live equivalent)
   ============================================================ */
// SUPABASE: insert workbook + its first sheet row.
...
// SUPABASE: update rally_sheet_workbooks set name where id.
...
// SUPABASE: delete workbook (cascade sheets + cells).
export function deleteWorkbook(id) {
  commit({ ...state, workbooks: state.workbooks.filter((w) => w.id !== id) });
  return { ok: true };
}

// SUPABASE: insert sheet row.
export function addSheet(wbId, name) {
...
export function setActiveSheet(wbId, sheetId) {
  commit({ ...state, workbooks: state.workbooks.map((w) => w.id === wbId ? { ...w, activeSheetId: sheetId } : w) });
}

/* Grow the grid. Real, deterministic - no re-render trickery. */
export function addRows(wbId, sheetId, n = 10) {
...
export function addCols(wbId, sheetId, n = 1) {
...
/* The core write: patch one cell. patch may carry any of
   { raw, bold, align, fmt, bg }. A cell that ends up fully empty
   (no content and no formatting) is dropped to keep state lean.
   SUPABASE: upsert rally_sheet_cells (workbook_id, sheet_id, ref). */
export function setCell(wbId, sheetId, cellId, patch) {
...
export function clearCell(wbId, sheetId, cellId) {
  const workbooks = state.workbooks.map((w) => {
    if (w.id !== wbId) return w;
    return { ...w, sheets: w.sheets.map((s) => { if (s.id !== sheetId) return s; const cells = { ...s.cells }; delete cells[cellId]; return { ...s, cells }; }) };
  });
  commit({ ...state, workbooks });
}
